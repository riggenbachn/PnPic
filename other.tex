...
\begin{remark}\label{rmk:naive_assoc_alg_description}
    ...
    This induces a functor $ p\colon BC_2 \times \underline{\mathrm{Fin}}_{*} \to \mathrm{Free}_{C_2,*} $. 
...
In particular, need a functor from $ \mathcal{LM}^\otimes $ into the fiber of $ \mathcal{HM}^\otimes $ over $ C_2/C_2 $. 
...
Then define a functor $ \Fun\left(\Delta^1, \Cat_\infty \right) \times_{\Cat_\infty} \Cat_{\infty,*} \to \left(\Cat_{\infty} \right)_{(-)^\op/\Spaces} $. 
