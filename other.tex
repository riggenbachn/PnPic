...
\begin{lemma}\label{lemma:stable_cat_functorial_Yoneda}
    Write $ \widetilde{\CAT^{\mathrm{ex}}} \to \CAT^{\mathrm{ex}} $ for the restriction of the universal cocartesian fibration to $ \CAT^{\mathrm{ex}} \subseteq \CAT $. 
...
\begin{proof}
    ...
    Write $ \mathrm{RFib} \subset \mathrm{Ar}\left(\CAT\right) \times_{\CAT} \CAT^{\mathrm{ex}} $ be the full subcategory of the arrow category spanned by the right fibrations. 
    ...
    The maps $ \mathrm{RFib}_* \to \CAT^{\mathrm{ex}} $, $ \mathrm{RFib} \to \CAT^{\mathrm{ex}} $ which send an arrow to its target are cocartesian fibrations; in particular by \cite[Corollary 6.5]{MR4545922}, the former is equivalent to the \emph{universal} cocartesian fibration $ \mathrm{RFib}_* \simeq \widetilde{\CAT^{\mathrm{ex}}} $. \Lucy{further discussion \href{https://mathoverflow.net/questions/383275/a-construction-of-the-universal-cocartesian-fibration}{here}} 
...
\end{observation}
\begin{definition}
    (\ref{diagram:calgp_to_hermitian_cat})
\end{definition}
\begin{theorem}
    The functor of (\ref{diagram:calgp_to_hermitian_cat}) factors through the subcategory $ \Catp \subseteq \Cath $. \Lucy{Use that $ \CAlgp $ (or $ \EE_\sigma \Alg $) has an initial object which is sent to $ \left(\Spectra^\omega,\Qoppa^u\right) $, so the image of any $ A \in \CAlgp $ under this functor has a canonical Poincaré object.}
\end{theorem}
\begin{proof}
    \cite[Lemma 3.4.3]{CDHHLMNNSI}
\end{proof}
...
\begin{remark}\label{rmk:E_sigma_operad_unital}
    The operad $ \underline{\Assoc}_\sigma^\otimes $ is unital in the sense of \cite[Definition 5.2.1]{NS22}. 
    Let $ \mathcal{C}^\otimes $ be a $ C_2 $-symmetric monoidal $ C_2 $-$ \infty $-category, and write $ \mathbbm{1} $ for the unit. 
    It follows from Proposition 5.2.8 \emph{ibid.} that there is an equivalence $ \EE_\sigma\Alg\left(\mathcal{C}^\otimes\right) \simeq \EE_\sigma\Alg\left(\mathcal{C}^\otimes\right)_{\mathbbm{1}/-} $. 
\end{remark}
\begin{observation}
