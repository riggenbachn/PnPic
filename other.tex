...
\begin{definition}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        If $ Y = \mathfrak{m} $, then $ \mathrm{Mul}_{\mathbf{BM}_\mathrm{inv}} \left(\{X_i\}_{i \in I}, Y\right) $ is the set of pairs consisting of a linear ordering $ \{i_1 < i_2 < \cdots < i_n\} $ on $ I $ and a function $ I \to \{\pm 1 \} $ if there is exactly one index $ i_k $ so that $ X_{i_k} = \mathfrak{m} $ and $ X_j = \mathfrak{a} $ for all $ j \neq i_k $, and $ \mathrm{Mul}_{\mathbf{BM}_\mathrm{inv}} \left(\{X_i\}_{i \in I}, Y\right) $ is empty otherwise. 
...
\begin{remark}\label{rmk:bimod_opd_involution}
    The colored operad $ \mathbf{BM}_{\mathrm{inv}} $ has a canonical involution $ \sigma $ which is the identity on objects and which sends a morphism $ (\ell, c) $ to $ (\ell^{\mathrm{rev}}, I \xrightarrow{c} \{\pm 1\} \xrightarrow{\cdot (-1)} \{\pm 1 \}) $. 
\end{remark}
...
\begin{remark}\label{rmk:bimod_opd_to_inv_bimod_opd}
    There is a map of colored operads $ \iota \colon \mathrm{BM} \to \mathrm{BM}_{\sigma} $ which sends $ \mathfrak{m} $ to $ \mathfrak{m} $ and sends both $ \mathfrak{a}_{\pm} $ to $ \mathfrak{a} $. 
    ...
    There is a map of colored operads $ \iota^{\mathrm{rev}} \colon \mathrm{BM} \to \mathrm{BM}_{\sigma} $ which sends $ \mathfrak{m} $ to $ \mathfrak{m} $ and sends both $ \mathfrak{a}_{\pm} $ to $ \mathfrak{a} $. 
...
\begin{remark}\label{rmk:inv_bimod_opd_to_inv_alg_opd} %compare Remark 4.2.1.5 in Higher algebra
    ...
    The map $ j $ induces a morphism of $ \infty $-operads $ \Associnv^\otimes \to \mathcal{BM}_{\mathrm{inv}}^\otimes $ which we will also denote by $ j $. 
    For any monoidal\Lucy{more general?} $ \infty $-category $ \mathcal{C} $, restriction along $ j $ sends an $ \EE_\sigma $-algebra $ A \colon \Associnv \to \mathcal{C}^{\otimes} $ to the pair $ (A, A) $ where $ A $ is regarded as an involutive\Lucy{hermitian} bimodule over itself. 
\end{remark} 
\begin{remark}\label{rmk:bimod_infty_opd_to_inv_bimod_infty_opd}
    ...
    Similarly to Remark \ref{rmk:assoc_infty_opd_to_assoc_inv_infty_opd}, there is a canonical natural equivalence $ \iota \simeq \iota^{\mathrm{rev}} $ of maps of $ \infty $-operads $ \mathcal{BM}^\otimes \to \BMinv^\otimes $. 
\end{remark}
\begin{remark}\label{rmk:rmod_lmod_opd_to_inv_bimod_opd}
    There are canonical maps of operads $ \mathcal{LM}^\otimes \to \mathcal{BM}^\otimes_{\mathrm{inv}} $ and $ \mathcal{RM}^\otimes \to \mathcal{BM}^\otimes_{\mathrm{inv}} $ making the diagram 
    \begin{equation*}
    \begin{tikzcd}[row sep=tiny]
        \mathrm{Assoc}^\otimes \ar[dd,"\sigma"] \ar[r] & \mathcal{LM}^\otimes \ar[dd,"{\mathrm{rev}}"] \ar[rd] & \\
        & & \mathcal{BM}^\otimes_{\mathrm{inv}}  \\ 
        \mathrm{Assoc}^\otimes  \ar[r] & \mathcal{RM}^\otimes \ar[ru] & 
    \end{tikzcd}   
    \end{equation*}
    commute, where $ \mathrm{rev} $ is the reversal involution of \cite[Remark 4.6.3.2]{LurHA}. 
\end{remark}
\begin{definition}
...
\begin{definition}
    ...
    We say that $ q $ exhibits $ \mathcal{O}_{\mathfrak{m}} $ as $ \EE_\sigma $-tensored over $ \mathcal{O}_{\mathfrak{a}} $ if $ q $ is a cocartesian fibration. 
\end{definition}
\begin{remark} % Compare Remark 4.2.1.21 in higher algebra
    ...
    By Remark \ref{rmk:bimod_infty_opd_to_inv_bimod_infty_opd}, we can think of $ q $ as giving an associative algebra $ \mathcal{C} $ in $ \mathrm{Cat}_\infty $ with an equivalence of algebras $ \sigma_{\mathcal{C}} \colon \mathcal{C} \simeq \mathcal{C}_{\mathrm{rev}} $, an $ \infty $-category $ \mathcal{M} $ equipped with both the structure of a $ \mathcal{C} $-bimodule (equivalently, the structure of a left $ \mathcal{C} \times \mathcal{C}_{\mathrm{rev}} $-module), and an involution $ \sigma_{\mathcal{M}} \colon \mathcal{M} \simeq \mathcal{M} $ which is linear with respect to the autoequivalence $ \mathcal{C} \times \mathcal{C}_{\mathrm{rev}} \xrightarrow{\mathrm{flip}} \mathcal{C}_{\mathrm{rev}} \times \mathcal{C} \xrightarrow{\sigma_{\mathcal{C}}^{-1} \times \sigma_{\mathcal{C}}}  \mathcal{C} \times \mathcal{C}_{\mathrm{rev}} $. 
\end{remark}
\begin{remark} 
    Let $ q \colon \mathcal{O}^\otimes \to \mathcal{BM}^\otimes_{\mathrm{inv}} $ be a cocartesian fibration of $ \infty $-operads. 
    Consider a hermitian module object $ F \colon \mathcal{BM}^\otimes_{\mathrm{inv}} \to \mathcal{O}^\otimes $. 
    By Remark \ref{rmk:rmod_lmod_opd_to_inv_bimod_opd}, $ F $ determines an associative algebra $ A $ of $ \mathcal{C} $ with an equivalence of algebras $ \sigma_A \colon A \simeq \sigma_{\mathcal{C}} (A)^{\mathrm{rev}} $, an object $ M \in \mathcal{M} $ so that both $ M $ and $ \sigma_{\mathcal{M}}(M) $ are equipped with the structure of a $ A $-$ \sigma_{\mathcal{C}}(A) $-bimodule. %and the structure of a right $ \sigma_{\mathcal{C}} (A) $-module on $ \sigma_{\mathcal{M}}(M) $. \Lucy{Want this to be a bimodule structure} 
    ...
    Furthermore, we have an equivalence $ \sigma_{M} \colon M \simeq \sigma_{\mathcal{M}} (M) $ which is linear with respect to the equivalence $ A \otimes \sigma_{\mathcal{C}}(A)^{\mathrm{rev}}  \xrightarrow{\mathrm{flip}} \sigma_{\mathcal{C}}(A)^{\mathrm{rev}} \otimes A \xrightarrow{\sigma_A^{-1} \otimes \sigma_A} A \otimes \sigma_{\mathcal{C}}(A)^{\mathrm{rev}} $. 
    \Lucyil{when $ \sigma_{\mathcal{M}} $ and $ \sigma_{\mathcal{C}} $ are both the identity (in particular, $ \mathcal{C}$ is equivalent to its opposite), I think this recovers the ``module with involution'' from \cite[\S3.1]{CDHHLMNNSI}.}
\end{remark}
...
\begin{remark}\label{rmk:nat_transformation_of_cut_functors} % compare remark 4.2.2.8
    ...
    \end{split}    
    \end{equation*} 
    This construction determines a morphism $ \gamma $ in the $ \infty $-category $ \Fun\left(\Delta^\op_\sigma, \mathcal{BM}^\otimes_{\mathrm{inv}}\right) $, or equivalently a map $ \gamma \colon \Delta^\op_\sigma \times \Delta^1 \to \mathcal{BM}^\otimes_{\mathrm{inv}} $. 
\end{remark}
...
\begin{definition}\label{defn:involutive_planar_module}
    ...
    Let $ \Modh^{\AA^\sigma_\infty}(\mathcal{M}) $ denote the full subcategory of $ \Fun_{\mathcal{BM}^\otimes_{\mathrm{inv}}}\left(\Delta^\op_\sigma \times \Delta^1,\mathcal{O}^\otimes\right) $ spanned by those maps $ f \colon \Delta^\op_\sigma \times \Delta^1 \to \mathcal{O}^\otimes $ satisfying 
    \begin{enumerate}
...
\begin{remark}
    Let $ q \colon \mathcal{O}^\otimes \to \mathcal{BM}^\otimes_{\mathrm{inv}} $ be a fibration of $ \infty $-operads, so $ q $ exhibits $ \mathcal{M}:= \mathcal{O}^\otimes_{\mathfrak{m}} $ as weakly bi-enriched over $ \mathcal{O}^\otimes_{\mathfrak{a}} $. 
    By \cite[Example 4.3.1.4 \& Proposition 4.3.2.15]{HTT}, composition with the inclusion $ \{0\} \to \Delta^1 $ induces a trivial Kan fibration $ \mathcal{M}^\varoast \xrightarrow{\sim} \mathcal{O}^\otimes \times_{\mathcal{BM}^\otimes_{\mathrm{inv}}} \Delta^\op_\sigma $. \Lucy{Jacob explains this in a really terse way--just by citing Prop 4.3.2.15 of HTT. It does just follow from definitions/observations, but there are many (for instance, definition of inert edge).}
...
\begin{lemma} % Compare Lemma 4.2.2.20 of Higher algebra
    Let $ q \colon \mathcal{O}^\otimes \to \mathcal{BM}^\otimes_{\mathrm{inv}} $ be a cocartesian fibration of $ \infty $-operads, so $ q $ exhibits $ \mathcal{M}:= \mathcal{O}^\otimes_{\mathfrak{m}} $ as tensored over $ \mathcal{O}^\otimes_{\mathfrak{a}} $. 
...
\begin{proposition}
    ...
    \begin{equation*}
        \Modh(\mathcal{M}) \simeq \Alg_{/\mathcal{BM}_{\mathrm{inv}}}(\mathcal{O}) \xrightarrow{\sim} \Modh^{\AA_\infty^\sigma}\left(\mathcal{M}\right)\,.
    \end{equation*}
\end{proposition}
\begin{proof}
...
\begin{proposition} \Lucy{This statement is \cite[Proposition 4.2.3.1]{LurHA} with some words changed; no claim of originality here.}
    ...
    Let $ K $ be a simplicial set so that $ \mathcal{M} $ admits $ K $-indexed limits, and let $ \theta \colon \Modh(\mathcal{M}) \to \Alg^\sigma(\mathcal{C}) $ be the forgetful functor. 
    Then
    \begin{enumerate}[label=(\arabic*)]
        \item For every commutative square 
        \begin{equation*}
        \begin{tikzcd}
            K \ar[r] \ar[d] & \Modh(\mathcal{M})  \ar[d,"\theta"] \\
            K^{\triangleleft} \ar[r] \ar[ru,dashed] & \Alg^\sigma(\mathcal{C}) \,,
        \end{tikzcd}
        \end{equation*}
        there exists a dashed arrow which is a $ \theta $-limit diagram. 
        \item An arbitrary map $ \overline{g} \colon K^{\triangleleft} \to \Modh(\mathcal{M}) $ is a $ \theta $-limit diagram if and only if the induced map $ K^{\triangleleft} \to \mathcal{M} $ is a limit diagram.   
    \end{enumerate}
\end{proposition}
\begin{proof}
    \Lucy{todo}
\end{proof}
\begin{corollary}
    $ \theta $ is a cartesian fibration, and a morphism $ f \colon \Delta^1 \to \Modh(\mathcal{M}) $ is $ \theta $-cartesian if and only if the image of $ f $ in $ \mathcal{M} $ is an equivalence.     
\end{corollary}
\begin{corollary}
    Let $ \mathcal{C} $ be an involutive monoidal $ \infty $-category and let $ \mathcal{M} $ be an $ \infty $-category which is bitensored over $ \mathcal{C} $. 
    Let $ K $ be a simplicial set so that $ \mathcal{M} $ admits $ K $-indexed limits, and let $ \theta \colon \Modh(\mathcal{M}) \to \Alg^\sigma(\mathcal{C}) $ be the forgetful functor. 
    ...
    Then
    \begin{enumerate}[label=(\arabic*)]
        \item $ \Modh_A(\mathcal{M}) $ admits $ K $-indexed limits. 
        \item A diagram $ K^{\triangleleft} \to \Modh_A(\mathcal{M}) $ is a limit diagram if and only if the induced diagram $ K^{\triangleleft} \to \mathcal{M} $ is a limit diagram. 
        \item Given a morphism $ A \to B $ of involutive algebra objects of $ \mathcal{C} $, the induced functor $ \Modh_B(\mathcal{M}) \to \Modh_A(\mathcal{M}) $ preserves $ K $-indexed limits. 
    \end{enumerate}
\end{corollary}
