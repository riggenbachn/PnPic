...
\end{definition} 
\begin{definition}
    Define a $ C_2 $-$ \infty $-operad $ \mathrm{Assoc}_{\mathrm{naive}}^\otimes \to \underline{\mathrm{Fin}}_{C_2, *} $ as having 
    \begin{itemize}
        \item objects consist of those arrows $ U \to V $ so that $ C_2 $ acts freely on $ U $ and transitively on $ V $ 
        \item morphisms consist of spans (\ref{diagram:C2_operad_generic_span}) in $ \underline{\mathrm{Fin}}_{C_2, *} $, plus the data of a pair $ (f, \leq) $ and for all orbits $ T \subset X $ an ordering on the orbits in the preimage $ f^{-1}(T) \subseteq Z $. 
        \item composition is defined to agree with that in $ \underline{\mathrm{Assoc}}^\otimes $. \Lucy{Can we regard this as a slice category, maybe over $ C_2/e = C_2/e $ in $ \underline{\mathrm{Assoc}}^\otimes $? }
    \end{itemize}
...
\begin{definition}
    ...
    \begin{itemize}
        ...
        \item morphisms from $ (h : U \to V, \ell \colon U \to \{a, m\}) $ to $ (h : X \to Y, \lambda \colon X \to \{a, m\}) $ consist of spans (\ref{diagram:C2_operad_generic_span}) in $ \underline{\mathrm{Fin}}_{C_2, *} $, plus the data of a pair $ (f, \leq) $ and for all orbits $ T \subset X $, an ordering on the orbits in the preimage $ f^{-1}(T) \subseteq Z $. 
        These are required to satisfy the condition 
        \begin{itemize}
            \item if $ x \in X $ is so that $ \lambda(x) = m $, there exists at exactly one orbit in $ f^{-1}(x) $ which $ \ell $ sends to $ m $, which is maximal with respect to the ordering.  
        \end{itemize}
    \end{itemize}
    there is a canonical map to $ \underline{\mathrm{Fin}}_{C_2, *} $ which is an inclusion on objects and on morphisms forgets the ordering. 
\end{definition}
