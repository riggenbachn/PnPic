...
\end{construction} 
There is a canonical forgetful functor $ U \colon \Cat^{\Mod}_\infty \to \Cat_\infty $. % $ \Fun\left(\Delta^1, \Cat^{\Mod}_\infty \right) \times_{\Cat^{\Mod}_\infty} \Cat^{\Alg\Mod}_\infty \to \Fun\left(\Delta^1, \Cat_\infty \right) \times_{\Cat_\infty} \Cat_{\infty,*} $. 
...
\begin{lemma}\label{lemma:stable_cat_functorial_Yoneda}
    ...
    Consider the cocartesian fibration $ \Catex_{,(-)^\op/\Spectra} \to \Cat_\infty $ classified by the functor $ \Fun^{\mathrm{ex}}\left((-)^\op, \Spectra\right) $, with cocartesian transport given by left Kan extension. 
    Then there is a functor $ \widetilde{\Catex} \to \Catex_{,(-)^\op/\Spectra} $ over $ \Catex $ which preserves cocartesian edges. \Lucy{replace $ \Catex $ by `large' stable $ \infty $-categories}
\end{lemma}
\begin{proof}
    ...
    The maps $ \mathrm{RFib}_* \to \Catex $, $ \mathrm{RFib} \to \Catex $ which send an arrow to its target are cocartesian fibrations; in particular by \cite[Corollary 6.5]{MR4545922}, the former is equivalent to the \emph{universal} cocartesian fibration $ \mathrm{RFib}_* \simeq \widetilde{\Catex} $. \Lucy{further discussion \href{https://mathoverflow.net/questions/383275/a-construction-of-the-universal-cocartesian-fibration}{here}} 
    For $ \mathcal{C} \in \Catex $, write $ \mathrm{RFib}_*(\mathcal{C}) $ for the $ \infty $-category of right fibrations over $ \mathcal{C} $ with a terminal object. 
...
\begin{construction}
    % Suppose we have constructed a diagram
    Combining (\ref{diagram:calgp_to_functor_cat_with_pointed_target}) with Lemma \ref{lemma:stable_cat_functorial_Yoneda} and Remark \ref{rmk:from_param_operad_maps_to_alg_mod_maps}, we obtain a commutative diagram\Lucyil{replace $ \Catex $ by \emph{`large'} stable $ \infty $-categories, add remark about $ \Cat^\epoly $ earlier}
