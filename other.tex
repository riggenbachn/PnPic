...
\begin{remark}\label{rmk:assoc_inv_mon_cat_as_mon_cat_with_autoequiv}
    ...
    With the same notation as before, pullback along $ \iota $ (resp. $ \iota^{\mathrm{rev}} $) determines algebra objects $ u(A) $, $ u^{\mathrm{rev}}(A) $ of $ \mathcal{C} $ and $ \mathcal{C}_{\mathrm{rev}} $, respectively. 
    ...
    Then the associated involution $ \sigma_{\mathcal{C}} $ is the identity, and for any involutive algebra object $ A $ of $ \mathcal{D} $, $ \sigma_A $ is an equivalence $ u(A) \simeq u(A)^{\mathrm{rev}} $. 
\end{remark}
\begin{definition}
...
\begin{remark} % Compare Remark 4.2.1.21 in higher algebra
    ...
    Informally, we can think of $ q $ as giving an associative algebra $ \mathcal{C} $ in $ \mathrm{Cat}_\infty $ with an equivalence of algebras $ \sigma_{\mathcal{C}} \colon \mathcal{C} \simeq \mathcal{C}_{\mathrm{rev}} $, an $ \infty $-category $ \mathcal{M} $ equipped with both the structure of a left $ \mathcal{C} $-module and a left $ \mathcal{C}_{\mathrm{rev}} $-module, and an involution $ \sigma_{\mathcal{M}} \colon \mathcal{M} \simeq \mathcal{M} $ which is $ \sigma_{\mathcal{C}} $-linear. 
\end{remark}
\begin{remark} 
    Let $ q \colon \mathcal{O}^\otimes \to \mathcal{BM}^\otimes_{\mathrm{inv}} $ be a cocartesian fibration of $ \infty $-operads. 
    Consider a hermitian module object $ F \colon \mathcal{BM}^\otimes_{\mathrm{inv}} \to \mathcal{O}^\otimes $. 
    Informally, $ F $ determines an associative algebra $ A $ of $ \mathcal{C} $ with an equivalence of algebras $ \sigma_A \colon A \simeq \sigma_{\mathcal{C}} (A) $, an object $ M \in \mathcal{M} $ equipped with both the structure of a left $ A $-module and a left $ \sigma_{\mathcal{C}} (A) $-module, and an involution $ \sigma_{M} \colon M \simeq M $ which is $ \sigma_{A} $-linear. 
\end{remark}
