...
\end{remark}
\paragraph{Meta-commentary} There are three things we want to do: 
...
\begin{definition}\label{defn:operad_monoid_with_involution}
...
\begin{definition}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        If $ Y = \mathfrak{a} $, then $ \mathrm{Mul} \left(\{X_i\}_{i \in I}, Y\right) $ is the set of pairs consisting of a linear ordering on $ I $ and a function $ I \to \{\pm 1\} $ if $ X_i = \mathfrak{a} $ for all $ i $, and empty otherwise. 
        If $ Y = \mathfrak{m} $, then $ \mathrm{Mul} \left(\{X_i\}_{i \in I}, Y\right) $ is the set of pairs consisting of a linear ordering on $ I $ and a function $ I \to \{\pm 1 \} $ IF there is exactly one index $ i_k $ so that $ X_{i_k} = \mathfrak{m} $ and $ X_j = \mathfrak{a} $ for all $ j \neq i_k $, and $ \mathrm{Mul} \left(\{X_i\}_{i \in I}, Y\right) $ is empty otherwise. 
        \item The composition law on $ \mathbf{BM}_{\mathrm{inv}} $ is determined by the composition of linear orderings, with reversal of linear orderings according to Definition \ref{defn:operad_monoid_with_involution} 
    \end{enumerate}
\end{definition}
\begin{definition}
...
\end{remark}
\begin{construction}
    Define a functor $ \mathrm{Cut} \colon \Delta_\sigma^\op \to BM^\otimes $: \Lucy{maybe this overloaded notation is not good. I'm running out of ideas.}
    \begin{itemize}
        \item For each $ ([n],s) $, we have $ \mathrm{Cut}([n],s) = \langle n + 1 \rangle \simeq \mathrm{RCut}_0([n]) $ where $ \mathrm{RCut} $ is from \cite[Construction 4.8.4.4]{LurHA}. 
        \item Given a morphism $ \alpha\colon ([n],s) \to ([m], t) $, the associated morphism $ \mathrm{Cut} ([n],s) \to \mathrm{Cut}([m], t) $ consists of
        \begin{itemize}
            \item On underlying finite pointed sets $ \langle m+1\rangle \to \langle n+1 \rangle $, $ \mathrm{Cut} $ agrees with (the reverse of) that appearing in \cite[Construction 4.2.2.6]{LurHA}
            ...
            Define $ u\colon \mathrm{Cut}(\alpha)^{-1}\left(\langle n+1\rangle^\circ\right) \to \{\pm 1\} $ to be the unique function so that $ u(j)t(j) = s(\mathrm{Cut}(\alpha)(j)) $. 
            % This means that $ \alpha^{-1}(\{k \mid k< j\}) $ and $ \alpha^{-1}(\{k \mid k \geq j\}) $ 
        \end{itemize}
    \end{itemize}
\end{construction}
