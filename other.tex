...
\begin{definition}\label{defn:normedrings}
    ...
    \begin{tikzcd}[column sep=small]
        & \Fun\left(\Delta^1,\EE_\infty\Alg\left(\Spectra\right)^{BC_p}\right) \ar[dd,"{\ev_{0},\ev_{1}}"]  & \\
        & & \Fun\left(\Delta^1,\EE_\infty\Alg\left(\Spectra^{C_p}\right)\right) \ar[dd,"{\ev_{0}, \ev_{1}}"] \ar[lu,"{(-)^e}"'] \\
        &\EE_\infty\Alg(\Spectra)^{BC_p} \times \EE_\infty\Alg(\Spectra)^{BC_p} & \\
        \EE_\infty\Alg\left(\Spectra^{C_p}\right)  \ar[rr,"{N^{C_p}(-^e) \times \id}"'] \ar[ru] \ar[ruuu,"{m \circ (-^e)}", bend left=30] & &\EE_\infty\Alg\left(\Spectra^{C_p}\right) \times \EE_\infty\Alg\left(\Spectra^{C_p}\right) \ar[lu,"{(-)^e \times (-)^e}"]
    \end{tikzcd}
    \end{equation}
    ...
    Observe that the right-hand trapezoid commutes essentially by definition, and the leftmost triangle commutes because $ \left(N^{C_p} A \right)^e \simeq (A^e)^{\otimes p} $. 
    Consider the limit of the diagram 
    \begin{equation}
        \CAlgp'\left(\Spectra^{C_p}\right) := \lim_K \mathcal{P} .
    \end{equation} 

    There is a canonical forgetful functor $ G' \colon \CAlgp\left(\Spectra^{C_p}\right) \to \EE_\infty\Alg\left(\Spectra^{C_p}\right)  $ given by the canonical projection to the lower left corner of the diagram. 
\end{definition}
...
\end{observation}
\begin{remark}
    There is a map of $ C_2 $-$ \infty $-operads $ \Com_{\mathcal{O}_{C_2}^\simeq} \to \EE_p $. 
\end{remark}
\begin{construction}
    Define a functor $ \gamma \colon \EE_p \Alg(\Spectra^{C_2}) \to \CAlgp $ (similar to \cite[\S4.1]{LYang_normedrings}). 
\end{construction}
\begin{theorem}
    Let $ A \in \EE_\infty\Alg\left(\Spectra^{C_2}\right) $ % and consider the adjunction $ F \dashv G $ of Recollection \ref{recollection:freealg_colimit}. 
    \begin{enumerate}[label=(\arabic*)]
    \item The underlying $ C_2 $-spectrum of the free $ \EE_p $ algebra $ F(A) $ on $ A $ is given by 
    \begin{equation}%\label{eq:freenormedalg_formula}
    F(A) \simeq
    \begin{tikzcd}
    ...
    % \item There is a canonical $ \E_\infty $ ring map $ \eta_A \colon A \to GF(A) $ given by $ \id_{A^{\varphi C_p}} \otimes (\eta_{A^e}:\sphere^0 \to A^e) $ on geometric fixed points and the identity on underlying.
    \end{enumerate}
\end{theorem}
