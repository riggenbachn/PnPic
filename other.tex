...
\Lucyil{Define Azumaya algebras with involution in terms of algebras over $ \underline{\Assoc}_\sigma^\otimes $ instead? I think it is `better' to use $ \underline{\Assoc}_\sigma^\otimes $ instead of $ \mathcal{HM}^\otimes $. To define a functor from algebras over $ \underline{\Assoc}_\sigma^\otimes $ to Hermitian $ \infty $-categories, we can define a functor from $ \mathcal{LM}^\otimes $ to $ \underline{\Assoc}^\otimes_{\sigma,C_2/C_2} $, then use the same argument as in Construction \ref{cons:module_with_gen_involution_in_families}.} 
    ...
    Choose a $ C_2 $-set $ U = \{u, v \} $ on which $ C_2 $ acts freely and transitively, and fix an ordering $ u \leq v $ on $ U $. 
    Define a functor 
    \begin{equation*}
    \begin{split}
        \iota_m \colon \mathcal{LM}^\otimes & \to \left(\underline{\Assoc}_\sigma^\otimes\right)_{C_2/C_2} \\
         \left(\langle n \rangle, S \right) & \mapsto \left((\langle n \rangle^\circ \setminus S) \times U \sqcup S_+ \to C_2/C_2, \ell(i) = \begin{cases}
             a & i \in (\langle n \rangle^\circ \setminus S) \times U \\
             m & i \in S 
         \end{cases} \right) \\
