...
\begin{enumerate}[label=(\alph*)]
    ...
    \end{itemize}
    \item Show that the formula for the cocartesian pushforward along a map $ A \to B $ is something like $ - \otimes \left(B \otimes B^\op \right) \otimes_{B\otimes B^\op} B $. 
    \begin{itemize}
...
\begin{definition}\Lucy{This is just an imitation of \cite[Definition 4.1.1.1]{LurHA}, modified in accordance with ideas from \S5.4.2. }
...
\begin{remark}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item a map of sets $ s \colon \langle m \rangle \to \{\pm 1\} $
        \end{itemize}
        \item For each pair of morphisms
        \begin{equation*}
             \left(\beta \colon \langle \ell \rangle \to \langle m \rangle, \preceq_j, s\right) \qquad \left(\alpha \colon \langle m \rangle \to \langle n \rangle, \preceq_i, s\right) \,,
        \end{equation*}
        the composite is the triple $ \alpha \circ \beta, \preceq_j'' $ where $ \preceq_j'' $ is the ordering on $ (\alpha \circ \beta)^{-1}(\{i\}) $ so that if $ a,b \in \langle \ell \rangle $ so that $ \alpha (\beta(a)) = \alpha(\beta(b)) $, then $ a \preceq_j'' b $ if $ \beta(a) \preceq_i \beta(b) $ or $ \beta(a) =_i \beta(b) = i $ and $ a \preceq_i b $ if $ s(i) = 1 $ or $ a \succeq_i b $ if $ s(i) = -1 $. 
        \Lucy{Note that when $ s, t$ are identically zero, the resulting order $ \preceq_j''$ agrees with the lexicographic order defined in \cite[Remark 4.1.1.4]{LurHA}.}  
    \end{itemize}
\end{remark}
\begin{definition}\Lucy{not sure about this/also a bit confused about $\mathrm{Cut}$...return to this later.}
    Define a category $ \Delta_\sigma $ 
    \begin{itemize}
        \item objects are pairs $ ([n], s \colon \{0, 1, \ldots, n\} \to \{\pm 1\}) $
        \item a morphism from $ ([n], s \colon \{0, 1, \ldots, n\} \to \{\pm 1\}) $ to $ ([m], t \colon \{0, 1, \ldots, m\} \to \{\pm 1\}) $ is an order-preserving map $ [n] \to [m] $ in $ \Delta $. 
    \end{itemize}
\end{definition}
