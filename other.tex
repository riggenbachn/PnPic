...
\begin{construction}
	Let $\mathcal{C}\in \mathrm{Mod}_{A^{e}}-\mathrm{Mod}$, and let $\lamdba:A^e\to A^e$ denote the involution on $A^e$. We then get an $A^t$-linear category by taking $\mathcal{C}\to \mathcal{C}\otimes \lambda^*\mathcal{C}$, where $A^t$ is the Tate Poincar{\'e} structure associated to the ring spectrum with $C_2$-action $A$. This is initial among Poincar{\'e} structures and therefore we have a map $A^t\to A^L$, from which we get an element of $\mathrn{Mod}_{A^L}(\mathrm{Sp}^{C_2})-\mathrm{Mod}$ by $\mathcal{C}\mapsto \mathrm{Mod}_{A^L}\otimes_{\mathrm{Mod}_{A^t}}(\mathcal{C}\otimes \lambda^*\mathcal{C})$. 
\end{construction}
