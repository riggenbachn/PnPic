...
\begin{observation}
    ...
    it follows that 
    \begin{equation*}
        \lim_K \mathcal{P} \simeq \left(\EE_\infty \Alg\left(\Spectra^{C_2}\right)\right) \times_{\left(\EE_\infty\Alg(\Spectra)^{\Delta^2} \times_{\EE_\infty \Alg(\Spectra)} \EE_\infty\Alg(\Spectra)^{\Delta^1}\right)} \EE\Alg(\Spectra)^{\Delta^1 \times \Delta^1} \,.
    \end{equation*}
...
\begin{proposition}\label{prop:calgp_monadic}
    The forgetful functor $ G' \colon \CAlgp\left(\Spectra^{C_p}\right) \to \EE_\infty\Alg\left(\Spectra^{C_p}\right) $ of Definition \ref{defn:calgp_alt_diagram} is monadic. 
\end{proposition}
\begin{proof}
...
\begin{proof}
    ...
    Write $ S \simeq S^f \sqcup S^t $ for the canonical decomposition where $ C_p $ acts freely on $ S^f $ and trivially on $ S^t $. 
    Then $ \displaystyle S \simeq S^t \sqcup \left(\bigsqcup_{Z \in \mathrm{Orbit}(S^f)} Z\right) \xrightarrow{\nabla \sqcup \left(\bigsqcup j_Z\right)} \to C_2/C_2 \sqcup U $ defines a morphism in $ \left(\mathrm{Com}_{\mathcal{O}_{C_2}^{\simeq,\op}}\right)_{C_2/C_2} $ so that the diagram
    \begin{equation*}
    \begin{tikzcd}
    ...
    \end{tikzcd}     
    \end{equation*} 
    commutes in $ \EE_p $; this is the desired map.  
\end{proof}
