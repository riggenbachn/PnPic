...
\end{lemma}
\begin{proof}
    \todo{need to define the category first!}
\end{proof}
...
\begin{lemma}\label{lemma:pointwise_units_has_left_adjoint}
    ...
    Denote the left adjoint by $ \sphere[-] $. 
\end{lemma}
\begin{proof}
...
Given an $ R $-linear $ \infty $-category $ \mathcal{C} $, we may consider its corresponding moduli stack of objects \cites{MR2493386}[\S5]{MR3190610}. 
There is a close relationship between objects in $ \mathcal{C} $ and Morita trivializations of $ \mathcal{C} $: The choice of an object $ G $ which is a compact generator of $ \mathcal{C} $ gives an equivalence $ \mathcal{C} \simeq \Mod_{\mathrm{End}_{\mathcal{C}}(G)}\left(\Mod_R\right) $. 
The space of compact generators comprises an open substack of the moduli stack of objects. \Lucy{cite \cite[Proposition 5.10]{MR3190610}?}
