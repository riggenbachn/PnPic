...
\begin{proposition}\label{prop:multiplicativity_of_Poincare_struct_from_eqvt_functor}
    ...
    \begin{tikzcd}
         \mathcal{W}_{\mathcal{D}}^\otimes \ar[r] \ar[d] & \left(\Cat\right)_{\op//\mathcal{D}^{BC_2}}^\otimes \ar[d] \\
...
\begin{construction}\label{cons:functor_classifying_twisted_diagonal} \Lucy{This is basically Construction 3.1.4 of \cite{CHN2024} with minor edits; we could possibly include less detail later.}
...
\begin{proof}
    ...
    \begin{equation}
    \begin{tikzcd}
         \mathcal{W}_{\mathcal{D}}^\otimes \ar[d] \ar[r] & \left(\Cat_{\op//\mathcal{D}^{BC_2}}\right)^\otimes \ar[d] \\ 
         \left(\Cat^{BC_2}\right)_{\op//\mathcal{D}}^\otimes \ar[r,"{\mathcal{C}^\otimes \mapsto \mathcal{C}}"] & \Cat^\times      
    \end{tikzcd}    \,,
    \end{equation}
...
\begin{observation}
    ...
    \begin{tikzcd}
        \mathcal{W}_{\mathrm{ex}}^\otimes \ar[r] \ar[d] & \Cath^\otimes \ar[d] \\
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item The presheaf $ \underline{\mathcal{O}} $ of \ref{lem_item:structure_sheaf_of_Green_func_is_functor} defines a Zariski sheaf. 
\end{enumerate}
\end{lemma}
\Lucy{The following is very speculative.}
\begin{construction}\label{cons:C2_mod_over_sheaf_of_Green_func}
    ...
    \Lucy{I want to use this to say that $ E \mapsto E \otimes \sigma(E) $ defines a `norm' functor from $ \Mod_{\mathcal{O}_X} \to \Mod_{\underline{\mathcal{O}}} $. 
    Then the quadratic functor we're interested in is $ \hom_{\Mod_{\underline{\mathcal{O}}}}(N^{C_2}( -), \underline{\mathcal{O}})$.}
\end{construction} 
...
\begin{lemma}\label{lemma:globalize_relative_norm}
...
\end{lemma}
\begin{proof}
    For each affine open $ \Spec A \subseteq Y $, write $ B = \Gamma \mathcal{O}_{\Spec A \times_Y X} $. 
    Then the functor $ N^{C_2}_Y $ is the limit over all $ \Spec A \subseteq Y $ of the functors $ \pi_{\#}\mathcal{O}_X \Mod \to B\Mod(\Spectra) \xrightarrow{N^{C_2}} \Mod_{N^{C_2}B}(\Spectra^{C_2}) \xrightarrow{ -\otimes_{N^{C_2}B} (A \to B)} \Mod_{A \to B}(\Spectra^{C_2}) $, where the last map is base change along the map $ N^{C_2}B \to (A\to B) $ which is a structure map for the $ C_2 $-$ \EE_\infty $-algebra structure on $ A \to B $. 
    \Lucy{todo: rewrite this as a diagram in a functor category with fixed domain and target.} 
...
\end{proof}
\begin{definition}
    Let $ \pi \colon X \to Y $ be a scheme with involution $ \sigma \colon X \xrightarrow{\sim} X $ and a good quotient. 
    Define $ \Qoppa_{\sigma} $ to be the functor
    \begin{equation*}
    ...
    \end{equation*}
    By Lemma \ref{lemma:globalize_relative_norm} and \Lucy{composite of an exact functor and an $ m$-excisive functor is $m$-excisive?}, $ \Qoppa_\sigma $ is quadratic. 
\end{definition}
\Lucy{Next, want to identify the bilinear part of $ \Qoppa_{\sigma} $ with that from Observation \ref{obs:cross_effect_twisted_poincare_structure}...but I \emph{think} this raises the question, is $ \Mod_{\mathcal{O}_X} \to \Mod_{\pi_{\#}\mathcal{O}_X} $ fully faithful?}
