...
\begin{remark}\label{remark:restriction_of_schemes_with_involution}
    Suppose $ (X,\lambda, Y, p) $ is an object of $ \mathrm{qSch}^{C_2} $ and $ j \colon U \to Y $ is a quasi-compact open subscheme. \Lucy{Do I need quasi-compact assumption?}
...
\begin{notation}\label{notation:scheme_involution_basechange}
    ...
    If $ j \colon U \to Y $ is [ASSUMPTIONS], let us write $ \pi^*U $ for the tuple $ (X \times_Y U, j^*(\sigma), U, j^*(\pi)) $. 
    \Lucy{Is $ \pi^*U $ an object of $ \mathrm{qSch}^{C_2} $ over $ (X, \sigma, Y, \pi) $? If not, does that matter? If $ j $ is flat, $ X \times_Y U $ agrees with derived fiber product and $ U $ inherits $ C_2 $-$ \EE_\infty $ ``structure sheaf'' so can still define Poincaré module category. Also see Remark \ref{remark:restriction_of_schemes_with_involution}} 
    Then the assignment $ (j \colon U \to Y ) \mapsto \left(\Mod^\omega_{j^*X}, \Qoppa_{j^*\underline{\mathcal{O}}}\right) $ defines a functor valued in symmetric monoidal Poincaré $ \infty $-categories over $ \left(\Mod^\omega_X, \Qoppa_{\underline{\mathcal{O}}}\right) $. 
\end{notation}
\Lucy{I think both of the following lemmas have essentially the same proof, as soon as the definitions/preliminaries are established.}
