...
Let $ X $ be a scheme. 
Gabber showed that if $ X $ is quasi-compact and separated and admits an ample line bundle, the collection of sheaves of Azumaya $ \mathcal{O}_X $-algebras up to Morita equivalence is in bijection with the torsion subgroup of $ H^2_{\mathrm{ét}}(X; \mathbb{G}_m) $ \cite{MR611868}. 
...
\end{definition}
Toën shows that the assignment $ X \mapsto \mathrm{dBr}(X) $ is an étale sheaf and there is an isomorphism $ \pi_0 \mathrm{dBr}(X) \simeq H^1_{\mathrm{ét}}(X; \ZZ) \times H^2_{\mathrm{ét}}(X; \mathbb{G}_m) $ \cite[Corollary 2.12]{MR2957304}. 
...
In particular, Toën's result gives concrete interpretations/realizations of all/not necessarily torsion classes in $ H^2_{\mathrm{ét}}(X;\mathbb{G}_m) $, at the cost of using derived methods/considering derived objects. 
