...
\end{lemma}
\begin{proof}
    The result follows from the observation that limits in $ \EE_\infty \Alg \left(\Spectra^{C_2}\right) $ are computed in $ \Spectra^{C_2} $. 
\end{proof}
...
\begin{definition}\label{defn:quadratic_functor_from_good_quotient}
    ...
    By Lemma \ref{lemma:globalize_relative_norm} and the fact that the composite of an exact (1-excisive) functor and an $ m$-excisive functor is $m$-excisive (see \cite[\S2.2]{Kpoly}), $ \Qoppa_\sigma $ is quadratic. 
...
\end{definition}
\begin{example} 
    Suppose $ L = \mathcal{O}_Y $. \Lucy{maybe for the purposes of this example, take $ Y $ to be affine?}
...
\begin{lemma}\label{lemma:bilinear_parts_agree}
    ...
    Assume that the quotient map $ q \colon X \to Y $ is étale. \Lucy{Need to figure out the weakest possible assumption here. \href{https://mathoverflow.net/questions/169052/are-quotients-of-affine-schemes-by-finite-groups-faithfully-flat}{relevant MO post}.}
    Let $ \Qoppa_\sigma $ be the Poincaré structure on $ \perf_X $ of Definition \ref{defn:quadratic_functor_from_good_quotient}. 
    Then the bilinear part of $ \Qoppa_\sigma $ agrees with that of Observation \ref{obs:cross_effect_twisted_poincare_structure}. 
\end{lemma}
\begin{proof}
    ...
    This follows from Lemma \ref{lemma:fully_faithful_pushforward}.  
\end{proof}
\begin{remark}
