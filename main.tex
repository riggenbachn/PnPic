...
The anti-involution is said to be of type $ 1 $ if it acts by the identity on $ R = $ the center of $ A $, and of type 2 if \Lucy{separate the discussion into two parts or roll it into one?} 

First and Williams considered involutions on Azumaya algebras which are neither of type 1 or 2 in \cite{azumaya_involution}. 
\Lucy{involutive Brauer group and the exact sequence?}
...
\begin{proof} [Proof of Proposition \ref{prop:Poincare_modules_as_etale_sheaf_affine_spectral}]
    ...
    By Proposition 6.1.4 of \cite{CDHHLMNNSI}, it suffices to show that the relevant diagram is a limit diagram in $ \Cath $. \Lucy{later: idempotent completion?} 
