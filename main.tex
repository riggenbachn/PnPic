...
\begin{proof}
    ...
    The proofs of parts \ref{thmitem:poincare_ring_has_limits} and \ref{thmitem:poincare_ring_to_naive_ring_preserves_lims} are analogous to those of \ref{thmitem:poincare_ring_has_colimits} and \ref{thmitem:poincare_ring_to_ring_preserves_colims} and have been omitted. 
    ...
    \Lucyil{Show that $ U(-) $ preserves cofiltered limits? Honestly at this point I'm kind of ok with it being a detail left to the reader, considering the inclusion of Remark \ref{rmk:limits_of_Poincare_rings}.}
...
\begin{lemma}\label{lemma:restriction_functor_cat_as_fibrations}
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        If $ \mathcal{C} $ admits all $ \mathcal{K} $-indexed limits, then the fibers of $ d_1^* $ admit all $ \mathcal{K} $-indexed limits and for each morphism $ f \colon \alpha \to \beta $ in $ \mathcal{C}^{\Delta^1} $, the associated functor $ f^* \colon \mathcal{C}^{\Delta^2}_{\beta} \to \mathcal{C}^{\Delta^2}_{\alpha} $ preserves all $ \mathcal{K} $-indexed limits. \Lucy{did I reverse the arrows appropriately?}
    \end{enumerate} 
\end{lemma}
\begin{proof} 
...
\begin{definition}\label{defn:Poincare_ring_pointwise_units}
    Define a functor $ (-)^u \colon \CAlgp \to \Fun\left(\mathcal{O}_{C_2}^\op,\Spectra_{\geq 0}\right) $ by taking units `pointwise,' i.e. $ (-)^u $ sends a Poincaré ring $ (R,R^{\varphi C_2} \to R^{tC_2}) $ to the diagram $ \mathrm{gl}_1(R^{C_1}) \to \mathrm{gl}_1(R^e) $.  
\end{definition}
...
\begin{proof}
    ...
    It is clear that $ (-)^u $ preserves small filtered colimits, since they are computed in $ \Spectra^{C_2} $. \Lucy{refer to earlier thm/lemma?}
...
\begin{definition}
    Varying $ X \to Y $, Construction \ref{cons:C2_mod_over_sheaf_of_Green_func} and Observation \ref{obs:globalize_relative_norm} define a functor \Lucy{want: codomain consists of $ C_2 $-stable $ C_2 $\emph{-presentable} $ C_2 $-$ \infty $-categories}
    \begin{align*}
...
\begin{remark}\label{remark:Poincare_units_fiber_sequence}
    The pushout description of $\gmq$ induces a pullback of mapping spaces \Lucy{mapping spaces should be taken in $ \CAlgp $? define $ \gmq $ in terms of left adjoint $ \sphere[-] $ in Lemma \ref{lemma:pointwise_units_has_left_adjoint}? Use the latter to show that $ \gmq $ takes values in connective spectra!}
    \[
    \begin{tikzcd}
        \gmq(A) \ar[d] \ar[r] & \mathrm{Maps}_{\CAlg(\mathrm{Sp}^{C_2})}(\mathbb{S}\{x^{\pm 1}, y^{\pm 1}\}, A)\simeq \mathrm{gl}_1(A^e)\ar[d]\\
