...
\begin{defn}
    ...
    Let $R$ be a ring spectrum. A \emph{Poincaré structure} on $R$ is a symmetric monoidal Poincaré $\infty$-category $\qoppa: \operatorname{Mod}_R^\omega\rightarrow \operatorname{Sp}$. We call such a symmetric monoidal Poincaré $\infty$-category a \emph{Poincaré ring spectrum}. We will denote the full subcategory of $\operatorname{CAlg(Cat^p_\infty)}$ spanned by Poincaré ring spectra by $\operatorname{CAlg^p}$ and call it the \emph{$\infty$-category of Poincaré ring spectra}.
\end{defn}
