...
\newcommand{\Lucyil}[1]{\todo[color=cyan,inline]{\linespread{1}\footnotesize L: #1}}
...
In \cites{CDHHLMNNSI,CDHHLMNNSII,CDHHLMNNSIII}, Calmès--Dotto--Harpaz--Hebestreit--Land--Moi--Nardin--Nikolaus--Steimle introduce \Lucy{also cite Jacob} a framework for talking about objects of stable $ \infty $-categories equipped with the data of nondegenerate hermitian forms. 
...
\Lucy{Will `Azumaya algebra' refer to the classical/discrete ones or the derived/generalized ones?}
    \begin{longtable}{lll}
...
\begin{definition}
    ...
    Let $ M $ be an $ R $-module. We denote by $ \sigma^*M $ its restriction of scalars, i.e. the $ R $-module with underlying abelian group $ M $ and action $ r \star m = \sigma(r) \cdot m $, where $\cdot$ denotes the original action of $M$.
    Define the \emph{adjoint} of $ M $ to be the $ R $-module $ M^\dag := \hom_R \left(\sigma^* M, R\right)$. \Lucy{see 3.8-3.11 in \href{https://arxiv.org/pdf/2009.09124}{this paper}} 
    Also recall that for a compact $R$-module $M$, there is a canonical $ R $-linear isomorphism $ \left(M^\dag\right)^\dag \simeq M $. 
    Note that given two compact $ R $-modules $ M, N $, the adjoint satisfies $ M^\dag \otimes N^\dag \simeq (M\otimes N)^\dag $. 
    
    Let $ I $ be a compact $ R $-module.
...
Then the fiber of the map \[u \colon \Brp(A)\to \Br(A^e)\] can be naturally identified with $ \Pic \left(\mathrm{Mod}_{A^L}\left(\mathrm{Sp}^{C_2}\right) \right)  $. 
Moreover, the connecting map $ \Omega \Br(A^e) \simeq \Pic(A^e) \to \mathrm{fib}(u) $ is induced by the norm $ \Mod_{A^e} \to \Mod_{A^L} $, $ X \mapsto N^{C_2} X \otimes_{N^{C_2}A^e} A^L $, which on underlying spectra is given by $ X \mapsto X \otimes_A \sigma^* X $. 
...
Consequently we need only identify the fiber over a single point instead of the homotopy fiber. 
Consider $ \left(\Mod_{A^e}^\omega, \Qoppa\right) $ a point in the fiber of $ \theta $ over $ \Mod_{A^e}^\omega $. 
% The description of fiber of $ \theta $ over the point $ \Mod_{A^e}^\omega $ follows from 
...
Now suppose $ \left(\Mod_{A^e}^\omega, \Qoppa\right) $ and $ \left(\Mod_{A^e}^\omega, \Phi\right) $ are two points in the fiber of $ \theta $ over $ \Mod_{A^e}^\omega $. 
We know that $ \Qoppa $ and $ \Phi $ are associated to invertible $ A^L $-modules $ (M, N, N \to M^{tC_2}) $ and $ (M', N', N' \to (M')^{tC_2}) $, resp. 
An equivalence from $ \left(\Mod_{A^e}^\omega, \Qoppa\right) $ to $ \left(\Mod_{A^e}^\omega, \Phi\right) $ lying over $\Mod_{A^e}^\omega$ consists of an a natural equivalence $ \eta \colon \Phi \xrightarrow{\sim} \Qoppa $. 
Now the natural equivalence $ \eta $ is classified by an equivalence $(M, N, N \to M^{tC_2}) \simeq (M',N', N' \to (M')^{tC_2}) $ of $ A^L $-modules (Proposition \ref{prop:classify_R_linear_Poincare_functors}), hence the result. %\cite[Lemma 3.4.3]{CDHHLMNNSI}.
\end{proof}
...
\begin{example}\label{ex:pnbr_closed_point_ramified}
    ...
    Now if char $ k \neq 2 $, then $ \pic(\underline{k}) \simeq \pic(k)^{hC_2} \simeq (\ZZ \times B k^\times)^{hC_2} $ where the generator of $ C_2 $ acts on $ \pi_1 B k^\times $ by $ u \mapsto u^{-1} $. We then have that $H^1(C_2,k^\times)=\{x\in k^\times | x\cdot x^{-1}=1\}/\{y^{-1}/y|y\in k^\times\}=k^{\times}/(k^\times)^2=0$ since $k$ is algebraically closed.
    ...
    If char $ k \neq 2 $ then the map $ \pi_0 \pic(k) \to \pi_0\pic(\underline{k}) $ is $ \ZZ \xrightarrow{\cdot 2} \ZZ $. 
    On the other hand, if char $ k = 2 $, then the map $ \pi_0 \pic(k) \to \pi_0\pic(\underline{k}) $ is $ \ZZ \xrightarrow{n \mapsto (2n, n)} \ZZ \times \ZZ $. \Noah{Why is the characteristic away from 2 case zero? It looks to me like that map is the identity?}
    % -----------------------------------------------------------------------------------
...
\begin{definition}\label{defn:Azumaya_genuine_anti-inv}
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \item \label{defnitem:Azumaya_alg_gi_underlying} an $ (A \otimes_R A^\op)^{\otimes_R 2} $-linear equivalence $ \hom_{R \otimes R}(A \otimes_R A, R) \simeq A \otimes_R A^\op $
        ...
        Here we regard $ A^{tC_2} $, which is canonically a $ (A \otimes A)^{tC_2} $-module, as an $ A \otimes_R R^{\varphi C_2} $-module (resp. $ A^\op \otimes_R R^{\varphi C_2} $-module) via the twisted Tate-valued diagonal $ A \to (A \otimes A^\op)^{tC_2} $ (resp. $ A^\op \to (A \otimes A^\op)^{tC_2} $). 
        \item \label{defn_item:Azumaya_gen_inv_geom_fixpt} An equivalence of $ \left(A \otimes_R A^\op\right) \otimes_R R^{\varphi C_2} $-modules 
        \begin{equation*}
...
\begin{example}\label{ex:endomorphisms_of_poincare_object}
    Let $ (R, R \to R^{\varphi C_2} \to R^{tC_2} ) $ be a Poincaré ring, and let $ (P,q ) \in \mathrm{Pn}\left(\Mod_R^\omega, \Qoppa_R \right) $. \Lucy{What if I replaced $ \Qoppa_R $ by a shift $ \Qoppa_R^{[n]} $? Or $ \Qoppa_R \circ \Sigma^n $? see \cite[\S3.5]{CDHHLMNNSI}}

    Then $ A:= \mathrm{End}_R(P) $ admits a canonical lift to an $ \EE_1 $ with genuine involution over $ R $ with $ A^{\varphi C_2}:= \hom_R(P, R^{\varphi C_2}) $. 
    ...
    To exhibit \ref{defnitem:Azumaya_alg_gi_underlying}, observe that $ q^\dag $ induces a canonical $ A \otimes A^\op $-linear equivalence $ A = \mathrm{End}(P) \simeq \mathrm{End}(P^\vee) \simeq A^\vee $.  
    % Lucy: I THINK that even though $ A \simeq P \otimes_R P^\vee $ (hence there is a canonical equivalence $ A \simeq A^\vee $), that isn't $ A \otimes A^\op $-linear. 
    ...
    A similar argument with the linear part of $ \Qoppa $ shows that we have an equivalence $ N_A \simeq \hom_{R}(P, R^{\varphi C_2}) $. 
    \Lucy{continue. discuss consequences of being Morita trivial (distinguished `point' in $ N_A $)? can we identify $ \overline{P} $?}
...
\begin{proposition}\label{prop:classical_inv_to_genuine_inv}
    Let $ R $ be a discrete ring with a given $ C_2 $-action $ \lambda $, and suppose that the ramification locus in $ \Spec (R)/C_2 = \Spec(R^{C_2}) $ is empty. 
    Let $ A $ be a classical Azumaya algebra over $ R $ with an involution of type 2. 
    Regard $ R $ as a Poincaré ring spectrum $ \underline{R}^\lambda $ via Example \ref{ex:fixpt_Mackey_functor}. 

    Then there is a canonical Azumaya algebra with genuine involution over $ \underline{R}^{\lambda} $ whose underlying Azumaya algebra is $ A $. \Lucy{In what other cases does a lift exist trivially? wrt Tate Poincaré structure, suffices to take a trivialization of $ A $?}
\end{proposition} 
\begin{remark}
...
\end{proposition} 
\begin{corollary}
    Let $ (R, R \to R^{\varphi C_2} \to R^{tC_2}) $ be a Poincaré ring. \Lucy{connectivity hypothesis on $ R $?} 
    Let $ \left(\mathcal{C}, \Qoppa\right) $ be an invertible idempotent-complete $ R $-linear Poincaré $ \infty $-category. 
    Then $ \left(\mathcal{C}, \Qoppa\right) $ is of the form $ \left(\Mod^\omega_A, \Qoppa_A \right) $ for some Azumaya algebra over $ R $ with genuine involution.      
\end{corollary}
\begin{proof}
    By Proposition \ref{prop:Poincare_cat_as_module_cat}, it suffices to exhibit a Poincaré object $ (P, q) $ whose underlying object is a generator of $ \mathcal{C} $. 
...
\begin{proof}[Proof of Proposition \ref{prop:Poincare_cat_as_module_cat}]
    Similar to Example \ref{ex:endomorphisms_of_poincare_object}\Lucy{to-do}
\end{proof}
...
\begin{notation}\label{notation:poincare_ring_basechange}
    ...
    \end{split}    
    \end{equation*}
    where the map $ R^{\varphi C_2} \otimes_R S \to R^{\varphi C_2} \otimes_{R^{tC_2}} S^{tC_2} \to R^{tC_2} \otimes_{R^{tC_2}} S^{tC_2} \simeq S^{tC_2} $ is given by base change along the Tate-valued norm composed with the structure map $ R^{\varphi C_2} \to R^{tC_2} $. 
...
\begin{proposition}\label{prop:Poincare_modules_as_etale_sheaf_affine_spectral}
    Let $ (R, R\to R^{\varphi C_2} \to R^{tC_2}) $ be a Poincaré ring and assume that $ R^{\varphi C_2} $ and $ R $ are connective. 
    Then the assignment of Notation \ref{notation:poincare_ring_basechange} is a hypersheaf on the small étale site of $ R $. 
    \Lucy{Antieau--Gepner write \emph{stack} for `sheaf of categories' and \emph{sheaf} for `sheaf of spaces.' Do we want to use this convention?}
\end{proposition}
\begin{corollary}\label{cor:pnbr_as_etale_sheaf_affine_spectral}
    Let $ (R, R\to R^{\varphi C_2} \to R^{tC_2}) $ be a Poincaré ring and assume that $ R^{\varphi C_2} $ and $ R $ are connective. 
    Write $ \pnbr $ for the composite functor $ \mathrm{\acute{E}t}_R \xrightarrow{\Mod^p} \EE_\infty\Alg\left(\Catpidem_{R}\right) \xrightarrow{\pnbr} \Spectra_{\geq 0} $, where $ \Mod^p $ is from Notation \ref{notation:poincare_ring_basechange} and $ \pnbr $ is Definition \ref{definition:poincare_brauer_space}. 
    Then $ \pnbr $ is an \'etale sheaf. 
\end{corollary} 
\begin{proof}
    Follows from Proposition \ref{prop:Poincare_modules_as_etale_sheaf_affine_spectral} and the fact that $ \pnbr $ preserves all limits.  
\end{proof}
\begin{proposition}\label{prop:Poincare_modules_as_etale_sheaf_inv_scheme}
    Let $ (X, \lambda, Y, \pi) $ be a scheme with involution $ X $ and a good quotient $ Y $. 
    Then the assignment of Notation \ref{notation:scheme_involution_basechange} is a hypersheaf on the small étale site of $ Y $. 
\end{proposition}
\begin{proof} [Proof of Proposition \ref{prop:Poincare_modules_as_etale_sheaf_affine_spectral}]
...
\begin{notation}
    ...
    \end{equation}
    here the cokernel is taken in the category of sheaves of abelian groups on the small \'etale site of $ X $. 
