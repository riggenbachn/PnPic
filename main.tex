...
\begin{remark}\label{remark:Poincare_units_fiber_sequence}
    The pushout description of $\gmq$ induces a pullback of mapping spaces \Lucy{define $ \gmq $ in terms of left adjoint $ \sphere[-] $ in Lemma \ref{lemma:pointwise_units_has_left_adjoint}? Use the latter to show that $ \gmq $ takes values in connective spectra!}
    \[
    \begin{tikzcd}
        \gmq(A) \ar[d] \ar[r] & \mathrm{Maps}_{\CAlgp}(\mathbb{S}\{x^{\pm 1}, y^{\pm 1}\}, A)\simeq \mathrm{gl}_1(A^e)\ar[d]\\
        * \ar[r] & \mathrm{Maps}_{\CAlgp}(\mathbb{S}\{z\},A)\simeq \Omega^\infty\Qoppa_A(A) 
    \end{tikzcd}\,.
    \] 
\end{remark}
...
\begin{proof}
    ...
    We now define a natural transformation $\gmq(-)\to \Omega\Picp(-)$. 
    By Lemma \ref{lemma:unit_trivial_norm}\ref{lemma_item:units_have_unit_norm}, there is a commutative diagram 
    \begin{equation}\label{diagram:units_to_loops_pnpic_map_of_fib_seq}
