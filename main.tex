...
\end{definition}
\Lucyil{Can replace good quotient $ Y $ by the Deligne--Mumford stack $ X//C_2 $ \cite[Construction 4.41]{azumaya_involution}. This should in some sense correspond to the symmetric Poincaré structure of Example \ref{example:symmetric_poincare_structure}. }
\begin{remark}
...
\begin{proof}
    ...
    \Lucyil{Minor modification needed? If $ \mathrm{Br}(-) $ refers to derived Brauer (`prime'), there is an additional factor of $ H^1_{\acute{e}t}(-;\mathbb{G}_m) $ (see \cite[Corollay 7.14]{MR3190610}). 
...
\begin{lemma}\label{lemma:pushforward_rep'able_n_geometric_sheaves}
    ...
    Assume that 
    \begin{itemize}
        \item The map $ R \to S $ is an effective descent morphism \Lucy{isn't this implied by the latter?}
