...
\end{corollary}
\begin{proof}
    A hermitian functor $ \left(\Mod_A^\omega, \Qoppa_A\right) \to \left(\Mod_B^\omega, \Qoppa_B\right) $ is a pair $ (\phi ,\eta) $. 
    Note that the underlying $ R $-linear functor $ \phi \colon \Mod_A^\omega \to \Mod_B^\omega $ is given by $ - \otimes_A P $ for some $ A^e $-$ B^e $-bimodule $ P $ which, when regarded as a $ B^e $-module, is compact. 
