...
Then the fiber of the map \[\Brp(A)\to \Br(A^e)\] can be naturally identified with the cofiber $ \mathrm{cofib}\left(\Pic\left(\Mod_{A^e}(\Spectra)^{hC_2}\right) \to \Pic \left(\mathrm{Mod}_{A^L}\left(\mathrm{Sp}^{C_2}\right) \right) \right) $, where the latter cofiber is taken in connective spectra.
\end{corollary}
...
An equivalence from $ \left(\Mod_{A^e}^\omega, \Qoppa\right) $ to $ \left(\Mod_{A^e}^\omega, \Phi\right) $ consists of an $ A^e $-linear equivalence $ F \colon \Mod_{A^e}^\omega \xrightarrow{\sim} \Mod_{A^e}^\omega $ and a natural equivalence $ \eta \colon \Phi \circ F^{\op} \xrightarrow{\sim} \Qoppa $. 
Now $ F $ is classified by an invertible $ A^e $-bimodule $ \beta $ and the natural equivalence $ \eta $ is classified by an equivalence $ \beta \otimes_{A^L} (M, N, N \to M^{tC_2}) \simeq (M',N', N' \to (M')^{tC_2}) $ of $ A^L $-modules \cite[Lemma 3.4.3]{CDHHLMNNSI}.
\end{proof}
\begin{example}\label{ex:pnbr_closed_point_ramified}
    ...
    Then 
    \begin{equation*}
        \pi_0\pnbr(\underline{k}) = 0.
    \end{equation*}
    To see this, note that we have a fiber sequence \[\pic(k)\to \mathrm{cofib}\left(\pic(\Mod_{k}(\mathrm{Sp})^{hC_2})\to \pic(\Mod_{\underline{k}}(\mathrm{Sp}^{C_2}))\right)\to \pnbr(\underline{k})\] which on $\pi_\star$ produces the exact sequence \[\ldots\to \mathbb{Z}\to \mathrm{coker}(\mathbb{Z}\to \mathbb{Z}\times \mathbb{Z})\to \pi_0(\pnbr(\underline{k}))\to 0\] where $\pic(\Mod_{\underline{k}}(\mathrm{Sp}^{C_2}))\cong \mathbb{Z}\times\mathbb{Z}$ via the map sending an invertible $\underline{k}$-module $\mathcal{L}$ to the pair $(n_1,n_2)$ where $\mathcal{L}^e\simeq k[n_1]$ and $\mathcal{L}^{\phi C_2}=k[n_2]$, see \cite[Section 8.1]{Krause_picard} for an argument. The $\mathbb{Z}$-term coming from $\pi_0(\Pic(\Mod_k^{hC_2}))$ maps to the diagonal, and therefore we get a short exact sequence \[0\to \mathbb{Z}\to \mathbb{Z}\to \pi_0(\pnbr(\underline{k}))\to 0\] where the left hand map is injective since $\pnbr(\underline{k})=0$.
    \Lucy{Pretty sure this follows from Corollary \ref{cor:Poincare_Brauer_to_Brauer_fiber}; will return to this later. Also, maybe put this example somewhere else.}
\end{example}
...
\begin{notation}\label{notation:scheme_involution_basechange}
    ...
    \end{equation*}
\end{notation}
\Lucy{I think Propositions \ref{prop:Poincare_modules_as_etale_sheaf_affine_spectra} and \ref{prop:Poincare_modules_as_etale_sheaf_inv_scheme} have essentially the same proof--present them together?}
...
\begin{corollary}
    ...
    \begin{equation*}
        ...
        \end{cases}
    \end{equation*}
    \Lucy{If we believe what's in Example \ref{ex:pnbr_closed_point_ramified}, then $ \pi_0 \pnbr $ should be the pushforward of $ \ZZ $ from the mod 2 reduction of the branch locus (terminology from \cite{azumaya_involution})}
\end{corollary}
