...
\DeclareMathOperator{\Cat}{\mathcal{C}at} % small categories
\DeclareMathOperator{\CAT}{C\textsc{at}_\infty} % (not necessarily small) categories
...
\newcommand{\Noah}[1]{\todo[color=red!30]{N: #1}}
...
\maketitle
\Lucy{Potential alt. title: Involutions on derived Azumaya algebras and pairings of twisted derived categories}
\begin{abstract}
...
If $ \mathcal{A} $ and $ \mathcal{A}' $ are Morita equivalent, then $ \mathcal{D}(\mathcal{A}') $ and $ \mathcal{D}(\mathcal{A}) $ are equivalent in $ \mathrm{Pr}^L_{\mathcal{O}_X} $; in fact, if $ \alpha \in H^2_{\mathrm{\acute{e}t}}(X; \mathbb{G}_m) $ denotes the Brauer class of $ \mathcal{A} $, then $ \mathcal{D}(\mathcal{A}) $ is a derived variant of the $ \alpha $-twisted sheaves considered by Caldararu and Lieblich \cites{MR1887894}{MR2579390}. \Lucy{cite de Jong? de Jong also refers to Giraud} 
That $ \mathcal{A} $ is Azumaya implies that $ \mathcal{D}(\mathcal{A}) $ is invertible in $ \mathrm{Pr}^L_{\mathcal{O}_X} $ with respect to the $ \mathcal{O}_X $-linear tensor product. \Lucy{who is this observation due to?} 
...
Moreover, First and Williams observe that a quotient involves a choice/is extra data; from the perspective of the stacky quotient $ X \to X//C_2 $, the $ C_2 $-action on $ X $ is free. \Lucy{connect to `many choices of different Poincaré structures' on a given $ \infty $-category eventually}  
...
At the level of derived categories, an equivalence $ \mathcal{A} \simeq \mathcal{A}^\op $ induces an equivalence $ \Mod_{\mathcal{A}}^\omega \simeq \Mod_{\mathcal{A}}^{\op, \omega} $, or equivalently a perfect pairing $ \Mod_{\mathcal{A}}^\omega \times \Mod_{\mathcal{A}}^\omega \to \Mod_{\mathcal{O}_X}^\omega $. \Lucy{pulled this from \cite[Remark 3.1.8]{CDHHLMNNSI}. }
In \cites{CDHHLMNNSI,CDHHLMNNSII,CDHHLMNNSIII}, Calmès--Dotto--Harpaz--Hebestreit--Land--Moi--Nardin--Nikolaus--Steimle introduce a category of stable $ \infty $-categories with perfect pairings/duality, expanding upon an idea introduced in \cite{Lurie_Ltheory_notes}. 
A object of a \emph{Poincaré $ \infty $-category} $ \mathcal{C} $ with duality functor $ D \colon \mathcal{C} \xrightarrow{\sim} \mathcal{C}^\op $ is an object $ X$ of $ \mathcal{C} $ equipped with an equivalence $ q \colon X \xrightarrow{\sim} DX $ which exhibits $ X $ as a fix point of the duality functor. \Lucy{forward reference to something more precise/correct.} %admit an enhanced notion of an object which are equipped with the data of nondegenerate hermitian forms. 
When $ \mathcal{C} = \Mod_k^\omega $ and $ D = (-)^\vee [n] $ (where $ (-)^\vee $ denotes $ k $-linear duality), an object is represented by a perfect complex $ M $ of $ k $-vector spaces and a `Poincaré duality' isomorphism $ M \simeq M^\vee[n] $. 
...
\begin{notation}
    ...
    Let $R$ be an $\mathbf{E}_\infty$-ring spectrum. We will drop $\mathbf{E}_\infty$ from our notation and simply call $R$ a \emph{ring spectrum}. Moreover, we will denote the $\infty$-category $\CAlg(\Spectra)$ of commutative algebra objects in the $\infty$-category of spectra $\Spectra$ by $\CAlg$. \Lucy{I'm ok with the notation $ \CAlg $, but am concerned about dropping $ \Einfty $ because $ \mathbb{E}_1 $-ring spectra will eventually come up as well. }
\end{notation}
...
\begin{definition}[Definition 1.2.2, \cite{CDHHLMNNSI}]
    A hermitian infinity category is a pair $(\mathcal{C},\Qoppa)$ where $\mathcal{C}$ is a small stable infinity category and $\Qoppa:\mathcal{C}^\op\to \mathrm{Sp}$ is a quadratic functor. 
\end{definition}

A map of hermitian infinity categories $(f,\eta):(\mathcal{C},\Qoppa)\to (\mathcal{D},\Psi)$ is a functor $f:\mathcal{C}\to \mathcal{D}$ and a $2$-cell $\eta:\Qoppa\implies \Psi \circ f^\op$.

Associated to any such quadratic functor $\Qoppa:\mathcal{C}^\op\to \mathrm{Sp}$ we may form a bilinear functor $B_\Qoppa(x,y):= \Qoppa(x\oplus y)^{\text{red}}$. 

\begin{definition}[Definitions 1.2.2, 1.2.8, \cite{CDHHLMNNSI}]
    We say that the associated bilinear form $B_\Qoppa$ is (right) non-degenerate if the functor \[\mathcal{C}^\op\to \mathrm{Fun}(\mathcal{C}^\op,\mathrm{Sp})\] given by sending $y\mapsto B_\Qoppa(-,y)$ lands in the essential image of the Yoneda embedding. Such hermitian structures then define a duality functor $D_\Qoppa:\mathcal{C}^\op \to \mathcal{C}$ and we say that the hermitian infinity category $(\mathcal{C},\Qoppa)$ is a Poincar{\'e} infinity category if the evaluation map $\mathrm{id}_{\mathcal{C}}\implies D_\Qoppa D_\Qoppa^\op$ is an equivalence.  
\end{definition}

A map of Poincar{\'e} infinity categories is a map of hermitian infinity categories which further preserves the duality functors. We will use \[\mathrm{Cat}^p_\infty \subseteq \mathrm{Cat}^h_\infty\] to denote the categories of Poincar{\'e} and hermitian infinity categories, respectively. These categories have several desirable properties. We collect several of the key properties below.

\begin{theorem}[Theorem 5.2.7, Proposition 6.1.2, Proposition 6.1.4, Corollary 6.2.9 \cite{CDHHLMNNSI}]
    \begin{enumerate}
        \item Both $\mathrm{Cat}_\infty^p$ and $\mathrm{Cat}_\infty^h$ are complete and cocomplete, and the functors $\mathrm{Cat_\infty}^p\to \mathrm{Cat}_\infty^h\to \mathrm{Cat}_\infty^{\mathrm{ex}}$ are conservative and preserve all small (co)limits.
        \item There is a symmetric monoidal infinity operad $(\mathrm{Cat}_\infty^h)^\otimes$ lifting $\mathrm{Cat}_\infty^h$.
        \item The forgetful functor lifts to a symmetric monoidal map $f^*:(\mathrm{Cat}_\infty^h)^\otimes \to (\mathrm{Cat}_\infty^{\mathrm{ex}})^\otimes$.
        \item This symmetric monoidal structure restricts to a symmetric monoidal structure on $\mathrm{Cat}_\infty^p$.
        \item The symmetric monoidal structures on $\mathrm{Cat}_\infty^p$ and $\mathrm{Cat}_\infty^h$ are closed. 
    \end{enumerate}
\end{theorem}
...
% \begin{remark}
    ...
    In \cite[discussion immediately preceding Examples 5.4.10]{CDHHLMNNSI}, the 9 authors describe the additional data (referred to as a `genuine involution') needed to endow $ \Mod^\omega_R $ with a symmetric monoidal Poincaré structure. 
    In the following, we introduce an $ \infty $-category which we refer to as \emph{Poincaré ring spectra} whose objects are the $\mathbf{E}_\infty$-ring spectra with genuine involution of Calmès--Dotto--Harpaz--Hebestreit--Land--Moi--Nardin--Nikolaus--Steimle. 
    There is a natural symmetric monoidal functor from Poincaré ring spectra to Poincaré $ \infty $-categories. \Lucy{reference or omit this sentence} 
    We include examples throughout and discuss how ordinary commutative rings with involution can be regarded as Poincaré ring spectra (Example \ref{example:genuine_symmetric_poincare_structure}). 
    % Poincaré ring spectra are referred to as $\mathbf{E}_\infty$-\emph{ring spectra with genuine involution}.
% \end{remark}
\begin{recollection}\label{recollection:Tate_valued_norm}
    Write $ \mathcal{O}_{C_2} $ for the orbit category of $ C_2 $.  
    The \emph{Tate-valued norm} is a functor $ \CAlg\left(\Spectra^{BC_2}\right) \to \operatorname{Fun}\left(\mathcal{O}_{C_2}, \CAlg(\Spectra)\right) $ Definition 3.8 and Lemma 3.10 of \cite{LYang_normedrings}. 
    Note that there is a canonical functor $ \iota \colon \Delta^1 = [0<1] \to \mathcal{O}_{C_2} $ which sends $ 0 $ to $ C_2/e $ and $ 1 $ to $ C_2/C_2 $. 
    Via precomposition with $ \iota $, we will regard the Tate-valued norm as a functor $ \CAlg\left(\Spectra^{BC_2}\right) \to \operatorname{Fun}\left(\Delta^1, \CAlg(\Spectra)\right) $. 
\end{recollection}
\begin{definition}
    ...
    \Lucy{I think we should include something justifying the name.}
    ...
    % A ring spectrum equipped with a Poincaré structure will be called a \emph{Poincaré ring spectrum}. 
\end{definition}

We will denote objects of $\CAlgp$ by \Noah{Change the notation $A^{\Phi C_2}$ to something else to make it more understandlable to people who don't know genuine homotopy theory? Change underline of A notation?}$\underline{A}=(A,s:A^{\Phi C_2}\to A^{tC_2})$. 
...
\begin{lemma}\label{lemma:Poincare_ring_geom_fixpt}
    ...
    \end{tikzcd}
    \end{equation*}
    where $ U $ is the functor of Remark \ref{rmk:Poincare_ring_has_underlying_C2_spectrum_alg} and the upper horizontal arrow is the canonical functor in Definition \ref{definition:poincare_ring_spectrum}. %\Lucy{add reference to definition of $ \CAlgp $?}
\end{lemma}
\begin{proof}
...
% Commented out July 24th--this remark overlaps with Definition \ref{definition:poincare_ring_spectrum}
% \begin{remark}
...
\begin{observation}\label{observation:normed_C2_ring_forget_to_Poincare_ring}
    In view of \cite[Theorem 1.3 \& Definition 3.13]{LYang_normedrings}, there is a forgetful functor from $ C_2 $-$ \EE_\infty $-algebras in $ \Spectra^{C_2} $ to Poincaré rings which forgets the $ C_2 $-equivariance of the map $ R \to R^{\varphi C_2 } $. 
    Their underlying $ \Einfty $-algebras in $ \Spectra^{C_2} $ agree. 
\end{observation}

\begin{remark}
    \label{remark:poincare_ring_spectra_as_algebra_objects} \Lucy{redundant/overlap with Remark \ref{remark:poincare_ring_spectra_to_modules_with_Poincare_structure}}
...
\begin{proof}
    ...
    To prove part \ref{thmitem:poincare_ring_to_ring_preserves_lims}, it suffices to show that $ U(-) $ preserves cofiltered limits and finite limits separately. 
    Since limits in $ \CAlg(\Spectra^{C_2}) $ are detected on underlying spectra and geometric fixed points, it suffices to show that $ \Phi^{C_2} U(-) $ preserves cofiltered limits and finite limits. 
    We begin by showing that $ \Phi^{C_2} U(-) $ preserves finite limits. 
    Let us mostly keep the notation/setup of the proof of part \ref{thmitem:poincare_ring_has_colimits}, except we are now interested in exhibiting an extension defined on $ K^{\vartriangleleft} $, and $ K $ is a finite simplicial set. 
    We have an extension $ \overline{f'p} \colon K^{\vartriangleleft} \to \CAlg^{BC_2} $ which is a limit diagram. 
    Since $ K $ is finite and the Tate construction preserves finite limits as a functor $ \Spectra^{BC_2} \to \Spectra $, the composite $ g \circ \overline{f'p} $ is a limit diagram in $ \CAlg^{\Delta^1} $. 
    Taking $ \mathcal{E} = * $ in \cite[Proposition 4.3.1.5(2)]{HTT}, we see that the extension $ \overline{g'p} $ (which exists by the proof of \ref{thmitem:poincare_ring_has_limits}) in fact defines a limit diagram in $ \CAlg^{\Delta^2} $. 
    The desired result now follows from the fact that limits in functor categories are computed pointwise and Lemma \ref{lemma:Poincare_ring_geom_fixpt}. 
    \Lucyil{Show that $ U(-) $ preserves cofiltered limits? Honestly at this point I'm kind of ok with it being a detail left to the reader, considering the inclusion of Remark \ref{rmk:limits_of_Poincare_rings}. Ok what follows is a sketchier proof of \ref{thmitem:poincare_ring_to_ring_preserves_lims} which should work in all cases.}

    Observe that there is a commutative square
    \begin{equation*}
    \begin{tikzcd}
        \CAlg^{\Delta^2} \ar[d,"{d_0}"] \ar[r,"{d_1}"] & \CAlg^{\Delta^1} \ar[d,"{\delta_0}"] \\
        \CAlg^{\Delta^1} \ar[r,"{\partial_0}"] & \CAlg \,.
    \end{tikzcd}
    \end{equation*}
    In particular, $ d_0 $ takes $ d_1 $-cartesian morphisms to $ \partial_0 $-cartesian morphisms and for any $ \alpha \in \CAlg^{\Delta^1} $, $ d_0 $ takes limits in $ \CAlg^{\Delta^2}_{\alpha} $ to limits in $ \CAlg^{\Delta^1}_{\delta_0(\alpha)} $. 
    It follows from the dual of \cite[Proposition 4.3.1.9]{HTT} that if $ \overline{g'p} $ is an $ f $-limit, then $ d_0 \circ \overline{g'p} $ is a $ \partial_0 $-limit. 
    Therefore, the functor $ \CAlgp \to \Einfty \Alg \left(\Spectra^{C_2}\right) $ preserves all small limits. 

    Part \ref{thmitem:poincare_ring_to_ring_preserves_filt_colims} follows from Remark \ref{rmk:limits_of_Poincare_rings} and the fact that filtered colimits of algebras are computed in modules. 
...
\begin{lemma}\label{lemma:restriction_functor_cat_as_fibrations}
    Let $ \mathcal{C} $ be an $ \infty $-category, and let $ \mathcal{K} $ be a collection of simplicial sets. 
    \begin{enumerate}[label=(\alph*)]
        \item \label{lemma_item:restriction_functor_cocartesian} If $ \mathcal{C} $ has finite colimits, then the functor $ d_1^* \colon \Fun\left(\Delta^2, \mathcal{C}\right) \to \Fun\left(\Delta^1, \mathcal{C}\right) $ exhibits the source as a cocartesian fibration over the target. 
        If $ \mathcal{C} $ admits all $ \mathcal{K} $-indexed colimits, then the fibers of $ d_1^* $ admit all $ \mathcal{K} $-indexed colimits and for each morphism $ f \colon \alpha \to \beta $ in $ \mathcal{C}^{\Delta^1} $, the associated functor $ f_* \colon \mathcal{C}^{\Delta^2}_{\alpha} \to \mathcal{C}^{\Delta^2}_{\beta} $ preserves all $ \mathcal{K} $-indexed colimits. \Lucy{wrote out the condition instead of just saying `cocartesian transport.' Is it clear that $ \alpha, \beta $ are objects of $ \mathcal{C}^{\Delta^1} $ (i.e. morphisms in $ \mathcal{C} $) and $ f $ is a commutative square?}
        \item \label{lemma_item:restriction_functor_cartesian} The functor $ d_1^* $ exhibits the source as a cartesian fibration over the target. 
        If $ \mathcal{C} $ has finite limits, then $ d_1^* $ exhibits the source as a cartesian fibration over the target. 
        If $ \mathcal{C} $ admits all $ \mathcal{K} $-indexed limits, then the fibers of $ d_1^* $ admit all $ \mathcal{K} $-indexed limits and for each morphism $ f \colon \alpha \to \beta $ in $ \mathcal{C}^{\Delta^1} $, the associated functor $ f^* \colon \mathcal{C}^{\Delta^2}_{\beta} \to \mathcal{C}^{\Delta^2}_{\alpha} $ preserves all $ \mathcal{K} $-indexed limits. 
    \end{enumerate} 
\end{lemma}
\begin{proof} 
    % Let us begin with some notation/constructions/an observation which will be useful for the proofs of both parts of the lemma. 
    ...
    Let us prove \ref{lemma_item:restriction_functor_cocartesian}. 
    Consider the maps 
    \begin{equation*}
        \Delta^1 \simeq \Delta^1 \times \{0\} \xrightarrow{i_0} \Delta^1 \times \Delta^1 \xrightarrow{j_0} \Delta^2
    \end{equation*}
    where $ i_0 $ classifies the morphism $ 00 \to 10 $ and $ j_0 $ sends the edge $ 10 \to 11 $ (i.e. the unique nonidentity in $ \{1\} \times \Delta^1 $) to the identity at $ 2 $ in $ \Delta^2 $ and satisfies $ j_0(00) = 0 $, $ j_0(01) = 1 $. 
    Note that their composite is $ j_0 \circ i_0 = d_1 $. 
    ...
    Note that $ i^*_0 $ corresponds to taking the `source' $ \Fun\left(\Delta^1,\mathcal{C}^{\Delta^1}\right) \to \mathcal{C}^{\Delta^1} $. 
    ...
    Because of our previous observation that $ i^*_0 $ corresponds to the source functor on the arrow category of $ \mathcal{C}^{\Delta^1} $ and $ \mathcal{C}^{\Delta^1} $ has finite colimits, $ i^*_0 $ is a cocartesian fibration. 
    In particular, $ i^*_0 $-cocartesian transport along a morphism in $ \mathcal{C}^{\Delta^1} $ corresponds to taking a pushout in $ \mathcal{C}^{\Delta^1} $. 
    \Lucyil{I have omitted details here because this is pretty standard, see e.g. \href{https://www.epfl.ch/labs/hessbellwald-lab/wp-content/uploads/2019/09/repcartfib.pdf}{Example 5.5 in these notes}. Note to self: see blackboard photo from Tuesday July 22nd.} 
    Furthermore, $ j^*_0 $ is fully faithful and identifies $ \mathcal{C}^{\Delta^2} $ with its essential image, the full subcategory of $ \mathcal{C}^{\Delta^1 \times \Delta^1} $ on those functors which send the edge $ 10 \to 11 $ to an equivalence in $ \mathcal{C} $. 
    To prove part \ref{lemma_item:restriction_functor_cocartesian}, it suffices to show that the essential image of $ j^*_0 $ is closed under $ i^*_0 $-cocartesian transport. 
    This is true because colimits in functor categories are computed pointwise by \cite[Corollary 5.1.2.3]{HTT} and equivalences are stable under pushouts.  
...
\end{proof}
\begin{remark}\label{rmk:limits_of_Poincare_rings}
    Suppose given a diagram $ p \colon K \to \CAlgp $ where $ K $ is some simplicial set. 
    Let us use the notation of \ref{thmitem:poincare_ring_has_colimits}, with the modifications discussed in the proof of \ref{thmitem:poincare_ring_to_ring_preserves_lims}. 
    Then the limit of $ p $ is computed as follows: Take $ f' \circ p \colon K \to \CAlg^{BC_2} $ and extend it to a limit diagram $ \overline{f'p} \colon K^{\vartriangleleft} \to \CAlg^{BC_2} $. 
    Write $ \infty $ for the cone point in $ K^{\vartriangleleft} $ and $ R_\infty:= \overline{f'p}(\infty) $. 
    Now regard $ g \circ \overline{f'p} $ as a natural transformation $ \overline{\eta} $ of functors $ K \to \CAlg^{\Delta^1} $ from the constant functor at $ R_\infty \to R_\infty^{tC_2} $ to $ g \circ f' \circ p $. 
    We may lift this to a pointwise (in $ K $) $ f $-cartesian natural transformation $ \eta $ of functors $ K \to \CAlg^{\Delta^2} $ with target $ g'\circ p $. 
    Let us write $ q_\infty \colon K \to \CAlg^{\Delta^2} $ for the domain of $ \eta $; it takes values in the fiber of $ f $ over $ R_\infty \to R_\infty^{tC_2} $. 
    Then the image of $ \lim p $ under $ g' $ can be identified with the limit of $ q_\infty $. \Lucy{Inspired by Remark 6.1.3 of \cite{CDHHLMNNSI}.} 

    An analogous description holds for colimits in $ \CAlgp $; we leave the details to the reader. 
\end{remark}
\Noah{Keep everything up to and including the above remark. Add in the identification of the adjoints of $\CAlgp\to \CAlg^{BC_2}$. Delete the last two things in this subsection.}
\begin{definition}\label{defn:Poincare_ring_pointwise_units}
    Define a functor $ (-)^u \colon \CAlgp \to \Fun\left(\mathcal{O}_{C_2}^\op,\Spectra_{\geq 0}\right) $ by taking units `pointwise,' i.e. $ (-)^u $ sends a Poincaré ring $ (R,R^{\varphi C_2} \to R^{tC_2}) $ to the diagram $ \mathrm{gl}_1(R^{C_2}) \to \mathrm{gl}_1(R^e) $. \Lucyil{Is this the `right' codomain for $ \mathrm{gl}_1 $/units of Poincaré rings? There \emph{is} a `transfer' $ \mathrm{gl}_1(R^{e}) \to \mathrm{gl}_1(R^{C_2}) $, it's just not guaranteed to be $ C_2 $-equivariant (where the target has the trivial $ C_2 $-action)} 
\end{definition}
\begin{lemma}\label{lemma:pointwise_units_has_left_adjoint}
    The functor $ (-)^u $ of Definition \ref{defn:Poincare_ring_pointwise_units} has a left adjoint. 
    Denote the left adjoint by $ \sphere[-] $. \Lucy{To define a functor $ \sphere[-] $ out of $ \Spectra^{C_2} $, precompose with the forgetful map $ \Spectra^{C_2}_{\geq 0} \to \Fun\left(\mathcal{O}_{C_2}^\op,\Spectra_{\geq 0}\right) $?}
\end{lemma}
\begin{proof}
    ...
    % \Lucyil{need to finish proof that $ (-)^{u} $ preserves small limits!}
\end{proof}
...
\begin{construction}\label{cons:C2_mod_over_sheaf_of_Green_func}
    ...
    \end{split}    
    \end{equation}
    where $ C_2\otimes\Cat $ denotes the $ \infty $-category of (small)\Lucy{bleh...cardinals} $ C_2 $-symmetric monoidal $ C_2 $-$ \infty $-categories and we regard $ \Mod_{(-)} $ as taking values in $ C_2 \otimes \Cat $ using \cite[\S A]{LYang_normedrings}. 
...
\begin{observation}\label{obs:globalize_relative_norm}
...
\begin{definition}
    Varying $ X \to Y $, Construction \ref{cons:C2_mod_over_sheaf_of_Green_func} and Observation \ref{obs:globalize_relative_norm} define a functor \Lucy{Use a functor from Poincaré rings/$ C_2 $-$ \Einfty $-algebras to (symmetric monoidal) Poincaré $ \infty $-categories to bypass $ C_2 $-$ \infty $-category stuff!} %\Lucy{want: codomain consists of $ C_2 $-stable $ C_2 $\emph{-presentable} $ C_2 $-$ \infty $-categories}
    \begin{align*}
...
\begin{split}
    ...
    \left(\mathcal{C}, L\right) &\mapsto (\mathcal{C}^e, \mathcal{C}^e \xrightarrow{N_{\mathcal{C}}} \mathcal{C}^{C_2} \xrightarrow{\hom_{\mathcal{C}^{C_2}}(-,L)} \Spectra)
\end{split}
\end{equation}
...
\begin{proof}
    Let $ \left(\mathcal{C}, L\right) $ and $ \left(\mathcal{D}, M\right) $ be objects of $ \widetilde{\mathrm{eInv}} $. 
    A morphism $ \left(\mathcal{C}, L\right) \to \left(\mathcal{D}, M\right) $ consists of a pair $ (F, \phi) $ where $ F \colon \mathcal{C} \to \mathcal{D} $ is a $ C_2 $-stable $ C_2 $-symmetric monoidal $C_2 $-functor and $ \phi \colon F(L) \simeq M $. 
\end{proof}
...
\Noah{Missing: Examples. Geometric fixed points supported on the ramification locus?}

\section{The Poincaré Picard space}
\label{section:the_poincare_picard_group}
\Noah{Decide on the notation for pnpin/picp/pnbr/brp, and also fix the difference between the group and the space(Looking at you Noah.).}

\Noah{Include and introduction to section 4.}

\Noah{Restructure 4, do the discrete case first, then do the poincar{\'e} ring case? Do some restructuring?}
...
In the usual Picard spectrum one has the relationship $\pic = \ZZ \times B\mathbb{G}_m$, where $\mathbb{G}_m$ is the spectral algebraic group scheme sending a ring spectrum $E$ to the spectrum of $E$-linear equivalences of $E$ $\mathrm{gl}_1E:=\mathrm{Aut}_E(E)$.\footnote{Normally the automorphism space of an object is only $\mathbb{A}_\infty$, but as the unit in a symmetric monoidal category, the automorphisms of $E$inherit a canonical and in fact functorial $\mathbb{E}_\infty$ structure and this construction makes sense.} 
\Lucy{Is $ \pic $ here referring to the picard space of the derived category or classical picard? Sort out with rest of notation later?}
...
We begin by defining an analogue of the multiplicative group/units for Poincaré rings. 
\begin{definition}
    Let $X$ be a Borel $C_2$ spectrum. We let $\mathbb{S}\{X\}$ denote the free $\mathbb{E}_\infty$ ring on the spectrum $X$. In the case of $X=\mathbb{S}[C_2]$, we will also denote $\mathbb{S}\{\mathbb{S}[C_2]\}=\mathbb{S}\{x,y\}$ for brevity. 
\end{definition}
\begin{construction}~\label{const: gmq} [Units of Poincaré rings] \Lucy{workshop the name...`hermitian/Poincaré/involutive units'?}
%	The underlying $\mathbb{E}_\infty$ ring of $\gmq$ will again be $\mathbb{S}\{x^{\pm 1}\}$, but in order to promote this ring to a Poincaré ring it will be helpful to write it as \[\mathbb{S}\{x^{\pm 1}, y^{\pm 1}\}\otimes_{\mathbb{S}\{z^{\pm 1}\}}\mathbb{S}\] where the map $\mathbb{S}\{z^{\pm 1}\}\to \mathbb{S}\{x^{\pm 1}, y^{\pm 1}\}$ is induced by $z\mapsto xy$. 
%    This ring naturally lifts to a Borel $C_2$-ring given by $C_2$ swaps $x$ and $y$ and does nothing to $z$. Now take $\gmq$ to be the Poincar{\'e} ring with underlying Borel $C_2$ structure as described above and geometric fixed points $(\gmq)^{\phi C_2}=\mathbb{S}$ and the map $(\gmq)^{\phi C_2}\to (\gmq)^{tC_2}$ given by the unit map. Endowing $(\gmq)^{\phi C_2}$ with the $\gmq$-module structure given by $x,y\mapsto 1$, it remains to show that the unit map $(\gmq)^{\phi C_2}\to (\gmq)^{tC_2}$ factors the Tate valued Frobenius $\gmq\to (\gmq)^{tC_2}$ in order to promote $\gmq$ to a Poincar{\'e} ring.
	
%	By construction of $\gmq$ this amounts to showing that on $\pi_0$ the Tate valued Frobenius sends $x,y\mapsto 1$ in $\pi_0((\gmq)^{tC_2})$. This map sends both $x$ and $y$ to $xy\in \pi_0((\gmq)^{tC_2})$. These are equal to $1$ in $\pi_0((\gmq)^{tC_2})$ since the functor $(-)^{tC_2}$ is lax-monoidal so $(\gmq)^{tC_2}$ is a module over $\mathbb{S}\{x^{\pm 1}, y^{\pm 1}\}^{tC_2}\otimes_{\mathbb{S}\{z\}^{tC_2}}\mathbb{S}^{tC_2}$ which has the image of $xy$ equal to $1$. 
    Consider the Tate Poincar{\'e} ring $\mathbb{S}\{x,y\}^t$. Let $R_\Qoppa$ denote the Poincar{\'e} ring which corepresents the functor sending a Poincar{\'e} ring $(S,S^{\phi C_2}\to S^{tC_2})$ to the space $\Omega^\infty S^{C_2}$. Note that both maps $\mathbb{S}\{x,y\}\to \mathbb{S}\{x,y\}^{tC_2}$ send both $x$ and $y$ to $0$ since both maps factor through the colimit comparison map \[\mathbb{S}\simeq \bigoplus_{n\geq 0}(\mathbb{S}[C_2]^{\otimes n}_{h\Sigma_n})^{tC_2}\to \left(\bigoplus_{n\geq 0}\mathbb{S}[C_2]^{\otimes n}_{h\Sigma_n}\right)^{tC_2}\simeq \mathbb{S}\{x,y\}^{tC_2}\] and therefore the element $xy$ lifts to an element of $\pi_0(\mathbb{S}\{x,y\}^{C_2})$. Thus we get a map of Poincar{\'e} rings $R_\Qoppa\to \mathbb{S}\{x,y\}$. Similarly there is a map of Poincar{\'e} rings $R_\Qoppa\to \mathbb{S}^u$ given by $1\in \pi_0(\mathbb{S}^{C_2})$. We then define \[\mathbb{G}_m^\Qoppa := \mathbb{S}\{x,y\}^t \otimes_{R_\Qoppa}\mathbb{S}^u\] where the tensor product is taken in Poincar{\'e} rings.
    \Lucy{later: different notation for the functor $ \gmq $ versus its [Poincaré] ring of functions.}
\end{construction}
...
\begin{proof}
    ...
    % Note that any automorphism of Hermitian objects will automatically be Poincar{\'e} and so we may instead describe the automorphisms as a Hermitian object. 
    ...
    Looping the aforementioned fibration twice, we have a fibration sequence %we may identify $ \mathrm{Aut}_{\mathrm{He}(\mathrm{Mod}_A^\omega,\Qoppa_A)}((A,u))$ with the fiber of the map 
    ...
    % In other words, an automorphism $(A,u)\to (A,u)$ is the data of an automorphism $a\in \mathrm{Aut}(A)$ together with a path $q $ from $ u $ to $ a^*u$ in $\Omega^{\infty}\Qoppa_A(A)$.  
	
	% There is a natural transformation $\gmq(-)\to \Omega\Picp(-)$ given as follows: we get a map $\gmq(A)\to \mathrm{Aut}_A(A)$ given by forgetting the Poincar{\'e} structure everywhere. 
    ...
    %By Lemma \ref{lemma:unit_trivial_norm}\ref{lemma_item:units_have_unit_norm}, there is a commutative diagram  \begin{equation}\label{diagram:units_to_loops_pnpic_map_of_fib_seq}
    %\begin{tikzcd}[row sep=tiny,column sep=large]
    %    \mathrm{gl}_1(A^e) \ar[r] \ar[d] & \mathrm{gl_1}\left((A^{L})^{C_2}\right) \ar[d,equals] \ar[r] & \Omega^\infty \Qoppa_A(A) \\ %\ar[d,equals] \\
    %    \mathrm{Aut}_A(A) \ar[r,"{f \mapsto f^*(u)}"] & \mathrm{gl_1}\left((A^{L})^{C_2}\right) %\Omega^\infty \Qoppa_A(A)
    %\end{tikzcd}
    %\end{equation}
    %where the upper horizontal arrow is from Remark \ref{remark:Poincare_units_fiber_sequence}. 
    %Now observe that the left vertical arrow in (\ref{diagram:units_to_loops_pnpic_map_of_fib_seq}) is an equivalence. 
    %The result follows from taking fibers over $ u $ in (\ref{diagram:units_to_loops_pnpic_map_of_fib_seq}) and comparing (\ref{eq:loops_pnpic_fiber_sequence}) with Remark \ref{remark:Poincare_units_fiber_sequence} and Lemma \ref{lemma:unit_trivial_norm}\ref{lemma_item:poincare_units_fib_seq}. \Lucy{Are Remark \ref{remark:Poincare_units_fiber_sequence} and Lemma \ref{lemma:unit_trivial_norm}\ref{lemma_item:poincare_units_fib_seq} slightly redundant?}
    % --------------------------------------------------------
    % It is enough to see that on $\pi_0$ the automorphisms of $A$ coming from $\gmq$ preserve $u$. 
    % By using the linear and quadratic decomposition of $\Qoppa_A$, for an element $a\in \pi_0(A)^\times$ send $u$ to $u$ is must be sent to $1\in \pi_0(A^{\phi C_2})$ and must act by $1$ on $A^{hC_2}$. 
    % By Lemma \ref{lemma:unit_trivial_norm}, this second condition is equivalent to $a\sigma(a)\in \pi_0(A)^\times$ being equal to $1$, but then these two conditions are exactly describing a map out of $\gmq$ as desired. 
	%
	% Consequently, we have a comparison map $\gmq(A)\to \Omega\Picp(\mathrm{Mod}_A,\Qoppa_A)$, and the above argument in fact shows that this map is an equivalence on $\pi_0$. 
    % To finish the argument, note that the pullback of Remark \ref{remark:Poincare_units_fiber_sequence} which finishes the proof. 
\end{proof}

\begin{lemma}\label{lemma:unit_trivial_norm}
    % DO NOT DELETE PLEASE--WORK IN PROGRESS. (July 21, 2025)
    Let $A\in \CAlgp(\mathrm{Sp})$.  
    In view of Remark \ref{rmk:Poincare_ring_has_underlying_C2_spectrum_alg},  there is a map of $ \infty $-groupoids 
    \begin{equation}\label{eq:Poincare_ring_congruence}     
        A^e \simeq \mathrm{End}_{A^e}(A^e) \xrightarrow{N^{C_2}} \mathrm{End}_{N^{C_2}A}\left(N^{C_2}A\right) \xrightarrow{\mathrm{ev}_u} \hom_{N^{C_2}A}\left(N^{C_2}A, A\right) \simeq \Omega^\infty \Qoppa_A(A) \simeq (A^L)^{C_2}
    \end{equation}   
    where $ u \colon N^{C_2}A \to A $ is the canonical map. 
    Write $ n_A $ for the composite of (\ref{eq:Poincare_ring_congruence}). Then
    \begin{enumerate}[label=(\arabic*)]
        \item \label{lemma_item:units_have_unit_norm} The composite $ \mathrm{gl}_1(A^e) \to A^e \xrightarrow{n_A} (A^{L})^{C_2} $ canonically factors through the inclusion $ {\mathrm{gl}_1\left((A^{L})^{C_2}\right) \to (A^{L})^{C_2}} $. 
        Under the identification $ \Omega^\infty \Qoppa_A(A) \simeq (A^L)^{C_2} $, $ u \in \Omega^\infty \Qoppa_A(A) $ is sent to $ 1 \in (A^L)^{C_2} $.
       \item \label{lemma_item:poincare_units_fib_seq} There is a fiber sequence
            \begin{equation*}
                \gmq(A) \simeq \mathrm{fib}_{1}(n_A) \to \mathrm{gl}_1(A^e) \xrightarrow{n_A} \mathrm{gl}_1\left((A^{L})^{C_2}\right)
            \end{equation*}
            where $ n_A $ denotes the map from \ref{lemma_item:units_have_unit_norm}. 
    \end{enumerate}
...
\end{lemma}
\begin{proof}
    The first half of part \ref{lemma_item:units_have_unit_norm} follows from the fact that functors send equivalences to equivalences. 
    The identification of $ u $ with $ 1 $ is by definition of $ \Qoppa_A $. \Lucy{add reference to $ \Mod^{\mathrm{p}} $ functor.} 
    \Lucyil{Rest of proof tbd, maybe overlap with Remark \ref{remark:Poincare_units_fiber_sequence}? Previous iteration is commented out but still appears in \texttt{.tex} file.}
...
\subsection{prime factorization and the picard group of hearts}\Viktor{still in development}\Noah{Need to remove the parts of this that need removal.}
...
\subsection{The discrete case: nondegenerate hermitian line bundles}
\label{subsection:the_discrete_case:nondegenerate_hermitian_forms}

Let $ R $ be a discrete commutative ring with a $ C_2 $-action $ \sigma \colon R \to R $, and let $ M $ be an $ R $-module. We denote by $ \sigma^*M $ its restriction of scalars, i.e. the $ R $-module with underlying abelian group $ M $ and action $ r \star m = \sigma(r) \cdot m $, where $\cdot$ denotes the original action of $M$. Since $\sigma^2=\text{id}_R$, restriction of scalars is isomorphic to extension of scalars, and thus restriction of scalars $\sigma^*:\text{Mod}_R\rightarrow \text{Mod}_R$ is symmetric monoidal in this case.

\begin{definition}
\label{definition:sigma-dual_of_module}
    Let $ R $ be a discrete commutative ring with a $ C_2 $-action $ \sigma \colon R \to R $, and let $M$ be an $R$-module. We define the $\sigma$-\emph{dual} of $ M $ to be the $ R $-module \Lucy{see 3.8-3.11 in \href{https://arxiv.org/pdf/2009.09124}{this paper}}  $$ M^\dag := \text{Hom}_R \left(\sigma^* M, R\right)=(\sigma^*M)^\vee.$$ 
\end{definition}

\begin{remark}
    \label{remark:restriction_of_scalars_commutes_with_dual}
    Let $R$ be a discrete commuative ring and let $\sigma:R\rightarrow R$ be an isomorphism of commutative rings such that $\sigma\circ\sigma=\operatorname{id}_R$.  Then we have a canonical isomorphism of $R$-modules $$\operatorname{Hom}_R(\sigma^* M, N)\simeq \sigma^*\operatorname{Hom}_R(M,\sigma_*N),$$ for any $R$-modules $N$ and $M$. In particular, we have 
    \begin{align*}
        M^\dag =(\sigma^* M)^\vee & \simeq \sigma^* (M^\vee),\\ 
        f &\mapsto \sigma\circ f 
    \end{align*}
    as $R$-modules. If $M$ is compact, we thus have a canonical isomorphism of $R$-modules $$c:M \simeq (M^\vee)^\vee\simeq \left(M^\dag\right)^\dag$$ $$m\mapsto \sigma\circ\operatorname{eval}_m.$$
    If both $M$ and $N$ are compact $R$-modules, then, using symmetric monoidalality of $\sigma^*:\text{Mod}_R\rightarrow \text{Mod}_R$, the $\sigma$-dual satisfies $$(M \otimes_R N)^\dag\simeq\sigma^*(M\otimes_R N)^\vee\simeq\sigma^*(M^\vee \otimes_R N^\vee)\simeq M^\dag \otimes_R N^\dag.$$
\end{remark}
    
\begin{definition}
    \label{definition:sigma-hermitian_form}
    Let $ R $ be a discrete commutative ring with a $ C_2 $-action $ \sigma \colon R \to R $, and let $ I $ be a compact $ R $-module. A \Viktor{nondegenerate? or why not poincare?} \emph{$ \sigma $-hermitian form on $ I $} is an $ R $-linear isomorphism $ \varphi \colon I \xrightarrow{\sim} I^\dag $ such that the following diagram commutes 
    \[\begin{tikzcd}
        I \ar[r,"\varphi"] \ar[d,"c", "\simeq"'] & I^\dag \\
        (I^\dag)^\dag \ar[ur,"\varphi^\dag"'] & .
    \end{tikzcd}\]
    We will denote a $\sigma$-hermitian form on $I$ by a pair $(I,\varphi)$.
    
    Let $I$ and $J$ be two compact $R$-modules with $\sigma$-hermitian forms $(I,\varphi)$ and $(J,\psi)$. A \emph{map of $\sigma$-hermitian forms}, $f:(I,\varphi)\rightarrow(J,\psi)$, is a map of $R$-modules $f:I\rightarrow J$ such that the following diagram commutes 
    \[\begin{tikzcd}
        I \ar[r,"\varphi"] \ar[d,"f"'] & I^\dag \\
        J \ar[r,"\psi"'] & J^\dag \ar[u, "f^\dag"']. 
    \end{tikzcd}\] We say $f$ is an isomorphism of $\sigma$-hermitian forms if its underlying $R$-module map is an isomorphism of $R$-modules.
\end{definition}
...
\begin{remark}
    \label{remark:invertible_hermitian_forms_are_locally_fixed_points_of_units}
    Let $R$ be a discrete commutative ring with a $C_2$-action $\sigma:R\rightarrow R$, and let $I$ be an invertible $R$-module. Let $\mathfrak{p}\subset R$ be a prime ideal. Since $I$ is locally free of rank $1$, a trivialization $\tau:I\otimes_R \sigma_*I\xrightarrow{\sim} R$ is locally of the form $$\tau_\mathfrak
    {p}:I_\mathfrak{p}\otimes_{R_\mathfrak{p}} \sigma_* I_\mathfrak{p}\xrightarrow{\sim } R_\mathfrak{p}\otimes_{R_\mathfrak{p}} \sigma_* R_\mathfrak{p}\xrightarrow{x_\mathfrak{p}} R_\mathfrak{p},$$ where the left isomorphism is given by the tensor product $t\otimes_{R_\mathfrak{p}}t$ of a trivialization $t:I_\mathfrak{p}\xrightarrow{\sim} R_\mathfrak{p}$ and the map on the right is given by the formula $r\otimes s\mapsto \sigma(s)\cdot x_\mathfrak{p} \cdot r$ for a unit $x_\mathfrak{p}\in R_\mathfrak{p}^\times$. The unit $x_\mathfrak{p}$ is unique up to an element $N(y)=y\sigma(y)\in R_\mathfrak{p}^\times$, where $y$ could be hiding behind the trivialization $t$. Since $R$ is commutative, the symmetry condition in Definition \ref{definition:sigma-hermitian_form} is then equivalent to requiring $x_\mathfrak{p}\in (R_\mathfrak{p}^\times)^{C_2}$, for each prime ideal $\mathfrak{p}\subset R$. In particular, when $\sigma=\text{id}_R$, $\tau$ is automatically symmetric. So the symmetry condition in Definition \ref{definition:sigma-hermitian_form} is vacuous for hermitian forms with underlying invertible module when the action on $R$ is trivial.
\end{remark}

\begin{proposition}
    \label{proposition:hermitian_picard_group_with_trivial_action_is_2-torsion}
    Let $R$ be a discrete commutative ring. 
    Then $\text{Pic}^\text{h}(R)$ is $2$-torsion. \Lucy{$ R $ has trivial $ C_2 $-action? Perhaps we can call this $ \mathrm{Pic}^s $ for `symmetric' forms?}
\end{proposition}

\begin{proof}
    \label{proof:hermitian_picard_group_with_trivial_action_is_2-torsion}
    Let $(I,\varphi)$ be an invertible hermitian form over $(R,\text{id}_R)$. Then the trivialization $I\otimes_R I\xrightarrow{\text{id}_R\otimes \varphi} I\otimes_R I^\vee\xrightarrow{<,>} R$ is an isomorphism of forms $(I\otimes_R I,\varphi\otimes_R \varphi)\simeq (R,u)$, which can be checked locally as in Remark \ref{remark:invertible_hermitian_forms_are_locally_fixed_points_of_units}. %To see this we must show that $f^\dagger\circ u\circ f=\varphi\otimes_R\varphi$, i.e. $\varphi(j)(i)\cdot \varphi(y)(x)=\varphi(i)(x)\cdot\varphi(j)(y)$, for all $i,j,x,y\in I$. Locally at a prime $\mathfrak{p}$ of $R$, $I$ is isomorphic to $R_\mathfrak{p}$ and thus $\varphi$ can be expressed by a unit $t$: $\varphi(a)(b)=bta$, for all $a,b\in R_\mathfrak{p}$. Then locally we have $\varphi(j)(i)\cdot \varphi(y)(x)=itjytx=\varphi(i)(x)\cdot\varphi(j)(y)$, for all $i,j,x,y\in I$. Thus we globally have $f^\dagger\circ u\circ f=\varphi\otimes_R\varphi$, implying $(I,\varphi)^{\otimes 2}\simeq (R,u)$. This proves the claim.
\end{proof}
...
\begin{remark}
    \label{remark:hermitian_picard_group_with_trivial_action_is_the_first_fppf_mu_2_cohomology group}
    Recall that we have an exact sequence of fppf sheaves $$0\rightarrow \mu_2\rightarrow \mathbf{G}_m\xrightarrow{\times 2} \mathbf{G}_m\rightarrow 0.$$ over $\mathbf{Z}$ and a canonical isomorphism $$\text{H}_\text{fppf}^1(X,\mathbf{G}_m)\simeq \text{Pic}(X),$$ for $X$ a scheme. 
    Let $R$ be a discrete commutative ring. Proposition \ref{proposition:split_exact_sequence_for_hermitian_picard_group_with_trivial_action} shows that we have the exact sequence $$\text{H}_\text{fppf}^{0}(R,\mathbf{G}_m)\xrightarrow{\times 2} \text{H}_\text{fppf}^{0}(R,\mathbf{G}_m)\rightarrow\text{Pic}^\text{h}(R)\rightarrow \text{H}_\text{fppf}^{1}(R,\mathbf{G}_m)\xrightarrow{\times 2} \text{H}_\text{fppf}^{1}(R,\mathbf{G}_m).$$ Hence, in the case of a trivial action on $R$, the hermitian Picard group is isomorphic to the first fppf cohomology group of $R$ with $\mu_2$-coefficients $$\text{Pic}^\text{h}(R)\simeq \text{H}_\text{fppf}^{1}(R,\mu_2).$$ If $R$ is moreover smooth over a Noetherian scheme of finite Krull dimension $S$ which is smooth over a Dedekind domain of mixed characteristic or over a field\Viktor{is this really the highest generality? }, by \cite[Proposition 10.9]{spitzweck-thesis}, this is isomorphic to motivic cohomology in degree $(1,1)$ with coefficients in $\mathbf{Z}/2$ $$\text{Pic}^\text{h}(R)\simeq \text{H}^{1,1}(R,\mathbf{Z}/2).$$
\end{remark}

The split exact sequence of Proposition \ref{proposition:split_exact_sequence_for_hermitian_picard_group_with_trivial_action} can be generalized to the case of a nontrivial $C_2$-action on $R$. To proof that, let us first establish two basic properties of the hermitian Picard group.

\begin{proposition}
    \label{proposition:hermitian_picard_group_is_nilinvariant}
    The hermitian Picard group $\text{Pic}^\text{h}$ is nilinvariant. More concretely, let $R$ be a discrete commutative ring with a $C_2$-action $\sigma:R\rightarrow R$ and let $R_\text{red}$ be the reduced ring of $R$ with $C_2$-action induced by the reduction map $R\rightarrow R_\text{red}$. Then the reduciton map induces an isomorphism $$\text{Pic}^\text{h}(R,\sigma)\xrightarrow{\sim} \text{Pic}^\text{h}(R_\text{red},\sigma).$$
\end{proposition}

\begin{proof}
    \label{proof:hermitian_picard_group_is_nilinvariant}
    This follows since the Picard functor of $\infty$-categories is nilinvariant. \Viktor{more details}
\end{proof}

\begin{proposition}
    \label{proposition:hermitian_picard_group_commutes_with_finite_sums}
    The hermitian Picard group $\text{Pic}^\text{h}$ commutes with finite sums. 
\end{proposition}

\begin{proof}
    \label{proof:hermitian_picard_group_commutes_with_finite_sums}
    \Viktor{todo}
\end{proof}

Given a discrete commutative ring $R$ and an isomorphism $\sigma:R\rightarrow R$, the $\sigma$-dual defines a $C_2$-action on the Picard group of $R$: 
\begin{align*}
    (-)^\dag:\text{Pic}(R) &\rightarrow \text{Pic}(R)\\
    [I] &\mapsto [I^\dagger]=[\sigma^* I^\vee]=-[\sigma^* I].
\end{align*} 
Given an invertible $\sigma$-hermitian form $\varphi:I\xrightarrow{\simeq} I^\dagger$, the $R$-module $I$ is a fixed point in the Picard group of $R$ with respect to this action. Let us denote the fixed points of $\text{Pic}(R)$ by this action by $\text{Pic}(R)^{-\sigma}$. We then have a forgetful map $$\text{Pic}^\text{h}(R,\sigma)\rightarrow \text{Pic}(R)^{-\sigma}.$$

\begin{notation}
    \label{notation:sigma-dual_fixed_points_of_picard_group}
    Let $R$ be a discrete commutative ring with a $C_2$-action $\sigma:R\rightarrow R$. The $\sigma$-dual defines a $C_2$-action on the Picard group of $R$ via $[I]\mapsto -[\sigma^*I]$. We will denote the fixed points of $\text{Pic}(R)$ of this action by $\text{Pic}(R)^{-\sigma}$.
\end{notation}

An invertible $R$-module $I$ is an element of $\text{Pic}(R)^{-\sigma}$ if and only if $[I]$ is in the kernel of the corresponding norm map: $N([I])=(\sigma +1)[I]=0$. This observation leads to the following generalization of Proposition \ref{proposition:split_exact_sequence_for_hermitian_picard_group_with_trivial_action}

\begin{proposition}
    \label{proposition:exact_sequence_for_hermitian_picard_group}
    Let $\text{Spec}(R)$ be a discrete irreducible affine scheme with a $C_2$-action $\sigma:R\rightarrow R$. Then we have the following split exact sequence $$0 \rightarrow (R^\times)^{C_2}/N(R^\times)\rightarrow\text{Pic}^\text{h}(R,\sigma)\rightarrow \text{Pic}(R)^{-\sigma}\rightarrow 0.$$ \Viktor{pich is likely not a zariski sheaf, but I believe there is an exact sequence for sheaves that can deal with intersections of irreducible components, which could explain the general case.}
\end{proposition}
...
\begin{remark}
    ...
    \[\begin{tikzcd}
         0\ar[r] & \mu_2(R)^{C_2} \ar[r] \ar[d] & (R^\times)^{C_2} \ar[r,"\times 2"] \ar[d] & (R^\times)^{C_2} \ar[r] \ar[d] & \text{Pic}^\text{h}(R^{C_2}) \ar[r] \ar[d] & \text{Pic}(R^{C_2}) \ar[r,"\times 2"] \ar[d] & 2\text{Pic}(R^{C_2}) \ar[d]\\
         0\ar[r] & (R^\times)^{-\sigma} \ar[r] & R^\times \ar[r,"N"] & (R^\times)^{C_2} \ar[r] & \text{Pic}^\text{h}(R,\sigma) \ar[r] & \text{Pic}(R) \ar[r,"N"] & \text{Pic}(R)^{C_2}
    \end{tikzcd}\]
    \Viktor{second row is not exact at the first pic as of now. all vertical maps are injective? Pic satisfies galois descent. Maybe so does $Pic^h$?}
\end{remark}

\begin{example}
    \label{example:hermitian_picard_group_is_not_a_zariski_sheaf}
    Let $R=k[x,y]/(xy)$. \Viktor{find example to show pich is not a zariski sheaf}
\end{example}

\begin{proposition}
    \label{proposition:pich_has_galois_descent}
    \Viktor{show galois descent for hermitian picard group}
\end{proposition}
...
\end{proof}
\Noah{To do: add in the case of schemes with good quotient.}

%\include{poincare_from_rigid}
\section{The Poincaré Brauer Group}
\Noah{Add a brief section reviewing the involuative brauer group of Paramla-Srinivas.}
\label{section:the_poincare_brauer_group}
In this section, we will define and study derived analogues of the involutive Brauer group. % for Poincaré ring spectra. 
In \S\ref{subsection:invertible_relative_Poincare_cats}, we define the involutive Brauer group in terms of invertible Poincaré $ \infty $-categories and compute it in a handful of examples. 
In particular, our examples (see Examples \ref{ex:pnbr_closed_pt_unramified} and \ref{ex:pnbr_closed_point_ramified}, especially the latter) show that our involutive Brauer group subsumes the classical involutive Brauer group but also captures information which is genuinely novel. \Lucy{add reference; workshop wording at end of sentence: `...captures non-discrete duality phenomena' or 'derived duality'?} 
Our definition will use the formalism of relative Poincaré $ \infty $-categories, some preliminary discussion of which is collected in \S\ref{subsection:relative_Poincare_cats_generalities}. 
\Lucy{revisit organization, then add exposition}
...
\Noah{Move this subsection to section 3.}
...
\begin{proposition}\label{prop:relative_poincare_cats_basic_properties}\Noah{Reorgonize this to be more readible.}
...
\begin{proof}
    ...
    \end{enumerate}
\end{proof}
If $ R $ is a Poincaré ring, then the $ \infty $-categories of $ R $-linear Poincaré and hermitian $ \infty $-categories are closed symmetric monoidal, similarly to their absolute counterparts \cite[Corollary 6.2.9, 6.2.15]{CDHHLMNNSI}. 
...
\begin{example}\label{ex:pnbr_closed_point_ramified}
    ...
    \Lucyil{This and Example \ref{ex:pnbr_closed_pt_unramified} are a pair; if we move one, also move the other. Remark \ref{rmk:non_classical_PnBr_classes} is about this example.}
...
\end{example}
\begin{remark}\label{rmk:non_classical_PnBr_classes}
    The 4-torsion class $ \left(\Mod_k^\omega, \Qoppa^{[1]}\right)$ (char $ k \neq 2 $) in Example \ref{ex:pnbr_closed_point_ramified} does not arise as perfect modules over a classical/discrete Azumaya algebra with anti-involution or central Wall anti-structure. 
    Roughly, the reason is: Any Poincaré object here cannot be concentrated in a single degree because the duality involves a shift, therefore the endomorphism algebra of any Poincaré generator will not be discrete either. 
    However, the 2-torsion class $ \left(\Mod_k^\omega, \Qoppa^{[1]}\right) \otimes  \left(\Mod_k^\omega, \Qoppa^{[1]}\right)  =  \left(\Mod_k^\omega, \Qoppa_{-k}\right) $ does arise as perfect modules over a classical Azumaya algebra with anti-involution. 
    Therefore, the subgroup of the $ \ZZ/4\ZZ $ which ``is classical'' is still 2-torsion! \Lucy{add reference/comparison to classical result. cite in intro?}
\end{remark}
...
\Noah{Add KR example if doable (Noah)}

\subsection{The Parimala-Srinivas fiber sequence}
\Noah{Change the notation for the norm map on pic.}
...
Writing this out for $A$ a discrete ring with $\frac{1}{2}\in A$, we have a fiber sequence \[\pic(A)\xrightarrow{N_{A^{C_2}/A}}\pic(A^{C_2})\to \pnbr(A)\] where we have that $N_{A^{C_2}/A}$ is given by the map $\mathcal{L}\mapsto (\mathcal{L}^{\vee}\otimes \sigma^*\mathcal{L}^\vee)^{C_2}$ which is (the negative of) the usual norm map. In order to extend this fiber sequence to the right we will need to categorify this map.
    ...
    \end{proof}
\Noah{Write up the correct version of this construction and the necessary things to prove in the other document.}
    \begin{construction}
    ...
    \end{construction}
\Noah{I am confident about everything in this section except the following theorem.}
    \begin{theorem}
...
Given an $ R $-linear $ \infty $-category $ \mathcal{C} $, there is a close relationship between objects in $ \mathcal{C} $ and Morita trivializations of $ \mathcal{C} $: The choice of an object $ G $ which is a compact generator of $ \mathcal{C} $ gives an equivalence $ \mathcal{C} \simeq \Mod_{\mathrm{End}_{\mathcal{C}}(G)}\left(\Mod_R\right) $. 
% we may consider its corresponding 
In the moduli stack of objects \cites{MR2493386}[\S5]{MR3190610} in $ \mathcal{C} $, the space of compact generators comprises an open substack. \Lucy{cite \cite[Proposition 5.10]{MR3190610}?}
Using this correspondence/philosophy, Antieau--Gepner show that the moduli stack of objects in $ \mathcal{C} $ is locally geometric, then use local geometricity of moduli stacks of objects to prove the existence of étale-local trivializations of invertible $ R $-linear $ \infty $-categories. 

In this section, we study analogous moduli stacks associated to $ R $-linear Poincaré $ \infty $-categories.   
In this setting, the appropriate replacement/analogy for an object is that of a \emph{Poincaré/hermitian object}. \Lucy{say something about conjectural adjunction between algebras and invertible modules with genuine involution and Poincaré $ \infty $-categories?}
