...
\begin{definition}
    ...
    Let $A$ be a Poincaré ring spectrum. We define the \emph{Brauer space of A} as $$\Brp(A):=\Pic(\Mod_A(\Catpidem)).$$
\end{definition}

Recall that a Poincaré $\infty$-category is called idempotent complete if the underlying stable $\infty$-category is idempotent complete. The full subcategory of $\Catp$ spanned by idempotent complete Poincaré $\infty$-categories is denoted by $\Catpidem$.
