...
Now $ F $ is classified by an invertible $ A^e $-bimodule $ \beta $ and the natural equivalence $ \eta $ is classified by an equivalence $ \beta \otimes_{A^L} (M, N, N \to M^{tC_2}) \simeq (M',N', N' \to (M')^{tC_2}) $ of $ A^L $-modules (Proposition \ref{prop:classify_R_linear_Poincare_functors}). %\cite[Lemma 3.4.3]{CDHHLMNNSI}.
\end{proof}
\begin{example}\label{ex:pnbr_closed_point_ramified}
    ...
    Now if char $ k \neq 2 $, then $ \pic(\underline{k}) \simeq \pic(k)^{hC_2} \simeq (\ZZ \times B k^\times)^{hC_2} $ where the generator of $ C_2 $ acts on $ \pi_1 B k^\times $ by $ u \mapsto u^{-1} $. \Lucy{leaving the action here since I wasn't sure at first about whether that term contributes to $\pi_0 \pic $ via the HFPSS (it does not).} 
    ...
    \begin{equation*}
        ...
        \end{cases}
    \end{equation*}
    by an argument similar \cite[Section 8.1]{Krause_picard}. 
...
\begin{example}\label{ex:pnbr_closed_pt_unramified}
    ...
    Now since $ \prod_{C_2} k $ is Borel, 
    \begin{equation*}
