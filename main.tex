...
\begin{definition}[Definition 1.2.2, \cite{CDHHLMNNSI}]
    A hermetian infinity category is a pair $(\mathcal{C},\Qoppa)$ where $\mathcal{C}$ is a small stable infinity category and $\Qoppa:\mathcal{C}^\op\to \mathrm{Sp}$ is a quadratic functor. 
\end{definition}

A map of hermetian infinity categories $(f,\eta):(\mathcal{C},\Qoppa)\to (\mathcal{D},\Psi)$ is a functor $f:\mathcal{C}\to \mathcal{D}$ and a $2$-cell $\eta:\Qoppa\implies \Psi \circ f^\op$.
...
\begin{remark}
    \label{remark:invertible_hermetian_forms_are_locally_fixed_points_of_units}
...
\begin{remark}
    ...
    Let $R$ be a discrete commutative ring. Proposition \ref{proposition:split_exact_sequence_for_hermetian_picard_group_with_trivial_action} shows that we have the exact sequence $$\text{H}_\text{fppf}^{0}(R,\mathbf{G}_m)\xrightarrow{\times 2} \text{H}_\text{fppf}^{0}(R,\mathbf{G}_m)\rightarrow\text{Pic}^\text{h}(R)\rightarrow \text{H}_\text{fppf}^{1}(R,\mathbf{G}_m)\xrightarrow{\times 2} \text{H}_\text{fppf}^{1}(R,\mathbf{G}_m).$$ Hence, in the case of a trivial action on $R$, the hermetian Picard group is isomorphic to the first fppf cohomology group of $R$ with $\mu_2$-coefficients $$\text{Pic}^\text{h}(R)\simeq \text{H}_\text{fppf}^{1}(R,\mu_2).$$ If $R$ is moreover smooth over a Noetherian scheme of finite Krull dimension $S$ which is smooth over a Dedekind domain of mixed characteristic or over a field\Viktor{is this really the highest generality? }, by \cite[Proposition 10.9]{spitzweck-thesis}, this is isomorphic to motivic cohomology in degree $(1,1)$ with coefficients in $\mathbf{Z}/2$ $$\text{Pic}^\text{h}(R)\simeq \text{H}^{1,1}(R,\mathbf{Z}/2).$$
\end{remark}

The split exact sequence of Proposition \ref{proposition:split_exact_sequence_for_hermetian_picard_group_with_trivial_action} can be generalized to the case of a nontrivial $C_2$-action on $R$. To proof that, let us first establish two basic properties of the hermetian Picard group.

\begin{proposition}
    \label{proposition:hermetian_picard_group_is_nilinvariant}
    The hermetian Picard group $\text{Pic}^\text{h}$ is nilinvariant. More concretely, let $R$ be a discrete commutative ring with a $C_2$-action $\sigma:R\rightarrow R$ and let $R_\text{red}$ be the reduced ring of $R$ with $C_2$-action induced by the reduction map $R\rightarrow R_\text{red}$. Then the reduciton map induces an isomorphism $$\text{Pic}^\text{h}(R,\sigma)\xrightarrow{\sim} \text{Pic}^\text{h}(R_\text{red},\sigma).$$
\end{proposition}

\begin{proof}
    \label{proof:hermetian_picard_group_is_nilinvariant}
    This follows since the Picard functor of $\infty$-categories is nilinvariant. \Viktor{more details}
\end{proof}

\begin{proposition}
    \label{proposition:hermetian_picard_group_commutes_with_finite_sums}
    The hermetian Picard group $\text{Pic}^\text{h}$ commutes with finite sums. 
\end{proposition}

\begin{proof}
    \label{proof:hermetian_picard_group_commutes_with_finite_sums}
    \Viktor{todo}
\end{proof}
...
\end{align*} 
Given an invertible $\sigma$-hermetian form $\varphi:I\xrightarrow{\simeq} I^\dagger$, the $R$-module $I$ is a fixed point in the Picard group of $R$ with respect to this action. Let us denote the fixed points of $\text{Pic}(R)$ by this action by $\text{Pic}(R)^{-\sigma}$. We then have a forgetful map $$\text{Pic}^\text{h}(R,\sigma)\rightarrow \text{Pic}(R)^{-\sigma}.$$
...
An invertible $R$-module $I$ is an element of $\text{Pic}(R)^{-\sigma}$ if and only if $[I]$ is in the kernel of the corresponding norm map: $N([I])=(\sigma +1)[I]=0$. This observation leads to the following generalization of Proposition \ref{proposition:split_exact_sequence_for_hermetian_picard_group_with_trivial_action}

\begin{proposition}
    \label{proposition:exact_sequence_for_hermetian_picard_group}
...
\begin{example}
    \label{example:hermetian_picard_group_is_not_a_zariski_sheaf}
    Let $R=k[x,y]/(xy)$. \Viktor{find example to show pich is not a zariski sheaf}
\end{example}

\begin{proposition}
    \label{proposition:pich_has_galois_descent}
    \Viktor{show galois descent for hermetian picard group}
\end{proposition}
