...
Also thank: Columbia, Ben Antieau...??
...
\begin{proposition}\label{prop:classical_inv_to_genuine_inv}
    Let $ R $ be a discrete ring with a given $ C_2 $-action $ \lambda $, and suppose that the branch locus in $ \Spec (R)/C_2 = \Spec(R^{C_2}) $ is empty. 
...
\begin{proof}
    ...
    \Lucyil{Here are some thoughts towards showing that the canonical map $ \colim_{[a,b]} \pi_* \mathbf{M}_{A^e} \to \pi_* \mathbf{M}_{A^e} $ is an equivalence. Later: see if \href{https://arxiv.org/pdf/2311.08051}{Example 3.1.2 here} could be useful?} 
    \begin{enumerate}
        ...
        \item Is $ \colim_{[a,b]} \pi_* \mathbf{M}_{A^e} $ still a hypersheaf? Hypersheaves are not in general closed under colimits, but maybe we can argue using an explicit model for this sheaf? 
        ...
        \item It suffices to show that the canonical map $ \colim_{[a,b]} \pi_* \mathbf{M}_{A^e} \to \pi_* \mathbf{M}_{A^e} $ is an equivalence on points; use the explicit model from \cite[Theorem 3.16]{azumaya_involution}?
    \end{enumerate}
    Since pushforward is left exact (and assuming that $ \pi_* $ preserves filtered colimits), it suffices to show that the pushforward of an $ n $-geometric sheaf on $ X $ is $ m $-geometric for some $ m $. 
\end{proof}
...
\begin{lemma}
    ...
    If either $ \pi \colon X \to Y $ is an isomorphism or quadratic étale on each connected component, then the inclusion $ \mathbf{Mor}^p_{A} \to \mathbf{M}^h_{A} $ exhibits $ \mathbf{Mor}^h_{A} $ as a quasicompact open subsheaf of $ \mathbf{M}^h_{A} $. 
...
\begin{proof}
    ...
    By the proof of Lemma \ref{lemma:hermitian_moduli_to_ordinary_moduli_is_geometric}, we know that $ (F,\eta) $ is given by $ (-\otimes M, \widetilde{p}^*A \to N^{C_2}_R(M^\vee)) $. 
    By the proof of \cite[Proposition 5.10]{MR3190610}, the subsheaf of points of $ \Spec S $ on which the functor $ F $ is an equivalence is the image $ U $ of a $ C_2 $-invariant quasicompact Zariski open under the quotient map $ \Spec R \to \Spec S $. 
    ...
    By \cite[Proposition 2.14]{MR3190610} (compare the proof of Proposition 5.10 of \emph{loc.cit.}), the subsheaf of points of $ \Spec S $ on which the map $ \tau_\eta $ is an equivalence is the image $ V $ of a $ C_2 $-invariant quasicompact Zariski open under the quotient map $ \Spec R \to \Spec S $. 
    Taking the intersection $ U \cap V $ gives the quasicompact Zariski open subsheaf of $ \Spec S $ on which $ (F,\eta ) $ defines a Poincaré Morita equivalence. 
\end{proof}
