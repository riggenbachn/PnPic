...
\begin{example}
    ...
    Let $R$ be a ring spectrum. If $2\in \pi_0(R)$ is invertible, we have $\underline{R}^{tC_2}\simeq 0$ \cite[Lemma I.2.8]{NS}. A Poincaré structure on $R$ with the trivial action is then given by an $R$-algebra $R\rightarrow C$.
\end{example}
...
\end{lemma} 
\begin{proof}
    \Lucy{todo--use Corollary \ref{cor:poincare_fourier_mukai}.}
\end{proof}
...
\begin{proof}
    \begin{enumerate}
        ...
        The locally $ \theta $-cocartesian maps are manifestly closed under composition, hence we are done. 
        \qedhere
    \end{enumerate}
\end{proof}
