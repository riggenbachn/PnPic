...
\end{corollary} 
\begin{proof}
    Follows from Proposition \ref{prop:Poincare_modules_as_etale_sheaf_affine_spectral} and the fact that $ \pnbr $ preserves all limits.  \Noah{Does $\pnbr$ preserve limits? This isn't clear to me...} 

    Here is an alternative proof. By the above we have that $\pic((-)^e):\mathrm{\acute{E}t}_Y^\op\to \mathrm{Sp}_{\geq 0}$ is an {\'e}tale hypersheaf, and similarly $\pic(\mathrm{Mod}_{(-)^L}(\mathrm{Sp}^{C_2})):\mathrm{\acute{E}t}_Y^\op\to \mathrm{Sp}_{\geq 0}$ is an {\'e}tale hypersheaf by the same argument as in Proposition~\ref{prop:Poincare_modules_as_etale_sheaf_affine_spectral}. Therefore $\pnbr:\mathrm{\acute{E}t}_Y^\op\to \mathrm{Sp}_{\geq 0}$ also is a hypersheaf by Corollary~\ref{cor:Poincare_Brauer_to_Brauer_fiber} since hypersheaves are closed under extensions.
\end{proof}
