...
\end{corollary}
\begin{proof}
Since $\theta:\mathrm{Mod}_{(\Mod_A^\omega, \Qoppa_A)}(\Catpidem)\to \mathrm{Mod}_{\mathrm{Mod}_A}(\Catex)$ is (co)cartesian, symmetric monoidal, and conservative, it follows that the induced map on the groupoid core of invertible objects is (co)cartesian as well. Since any (co)cartesian fibration between Kan complexes is a Kan fibration by \cite[Lemma 2.1.3.3]{HTT} and (the dual of) \cite[Proposition 2.4.2.4]{HTT}. Consequently we need only identify the fiber instead of the homotopy fiber, which follows from the identifiaction of the fiber in Proposition~\ref{propitem:classify_R_lin_hermitian_struct}. \Noah{Needs a lot more detail I know, but I think the skeleton is here.}
\end{proof}
