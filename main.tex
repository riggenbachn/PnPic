...
\begin{ex}
    ...
    Let $R$ be a ring spectrum. If $2\in \pi_0(R)$ is invertible, we have $\underline{R}^{tC_2}\simeq 0$\Viktor{explain/reference}. A Poincaré structure on $R$ with the trivial action is then given by the choice of an $R$-algebra $R\rightarrow C$.
\end{ex}
...
\begin{defn}
    ...
    \begin{itemize}
        \item \Viktor{unwind definitions and then define the $\infty$-category of poincare ring spectra $\operatorname{CAlg^p}$}
    \end{itemize}
\end{defn}
...
Let $A$ be a Poincaré ring spectrum. Then $A$ is an algebra object in the $\infty$-category of Poincaré $\infty$-categories $\operatorname{Cat_\infty^p}$\Viktor{ref}. We may thus consider the $\infty$-category of modules over it $\operatorname{Mod}_A(\operatorname{Cat_\infty^p})$.\Viktor{this is the beginning of an attempt to define brauer groups for poincare ring spectra}
