...
\begin{construction}~\label{const: gmq} [Units of Poincaré rings]
...
\begin{remark}\label{remark:Poincare_units_fiber_sequence}
    The pushout description of $\gmq$ induces a pullback of mapping spaces \Lucy{define $ \gmq $ in terms of left adjoint $ \sphere[-] $ in Lemma \ref{lemma:pointwise_units_has_left_adjoint}?}
    \[
    \begin{tikzcd}
...
\begin{proof}
    ...
    By Lemma \ref{lemma:unit_trivial_norm}, there is a commutative diagram
...
\subsection{The discrete case: nondegenerate hermetian line bundles}
...
\begin{remark}\label{rmk:non_classical_PnBr_classes}
    The 4-torsion class $ \left(\Mod_k^\omega, \Qoppa^{[1]}\right)$ (char $ k \neq 2 $) in Example \ref{ex:pnbr_closed_point_ramified} does not arise as perfect modules over a classical/discrete Azumaya algebra with anti-involution. \Lucy{or central Wall anti-structure}
