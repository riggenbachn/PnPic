...
\end{proposition}
\begin{proof}
    Recall that Proposition \ref{prop:relative_poincare_cats_basic_properties}.\ref{propitem:Rlin_Poincare_cats_is_symm_mon} furnishes an adjoint pair $ \Catp_R \leftrightarrow \Catp $ of functors. 
    Write $ \overline{\mathcal{C}} = (\mathcal{C},\Qoppa_{\mathcal{C}}) \in \Catpidem_R $. 
    Then
    \begin{equation*}
        \mathrm{Pn}(\mathcal{C}) = \hom_{\Catp}\left((\Spectra^f,\Qoppa^u), U(\overline{\mathcal{C}})\right) \simeq \hom_{\Catp_R}\left(\left(\Mod_R^\omega,\Qoppa_R\right)\otimes(\Spectra^f,\Qoppa^u), \overline{\mathcal{C}}\right) \,. \qedhere
    \end{equation*}
\end{proof}
