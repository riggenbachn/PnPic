...
\begin{proof}
    ...
    By Lemma \ref{lemma:unit_trivial_norm}, there is a commutative diagram \Lucy{edit Lemma accordingly}
    \begin{equation}\label{diagram:units_to_loops_pnpic_map_of_fib_seq}
    \begin{tikzcd}[row sep=tiny,column sep=large]
        \mathrm{gl}_1(A^e) \ar[r] \ar[d] & \Omega^\infty \Qoppa_A(A) \ar[d,equals] \\
        \mathrm{Aut}_A(A) \ar[r,"{f \mapsto f^*(u)}"] & \Omega^\infty \Qoppa_A(A)
    \end{tikzcd}
    \end{equation}
    ...
    The result follows from taking fibers over $ u $ in (\ref{diagram:units_to_loops_pnpic_map_of_fib_seq}) and Remark \ref{remark:Poincare_units_fiber_sequence} and (\ref{eq:loops_pnpic_fiber_sequence}). 
...
\begin{lemma}\label{lemma:unit_trivial_norm}
    ...
    % Let $A\in \CAlgp(\mathrm{Sp})$ and $s\in \mathrm{gl}_1(A) $. 
    % Then $a\sigma(a)=1$ in $\pi_0(A^{C_2})$ if and only if $(a\otimes a)^*$ acts by $1$ on $ A^{C_2}=\mathrm{Hom}_{N^{C_2}A}(N^{C_2}A, A^L) $.
