...
Now $ F $ is classified by an invertible $ A^e $-bimodule $ \beta $ and the natural equivalence $ \eta $ is classified by an equivalence $ \beta \otimes_{A^L} (M, N, N \to M^{tC_2}) \simeq (M',N', N' \to (M')^{tC_2}) $ of $ A^L $-modules \cite[Lemma 3.4.3]{CDHHLMNNSI}. \Noah{Needs a lot more detail I know, but I think the skeleton is here.}
\end{proof}
\begin{example}\label{ex:pnbr_closed_point_ramified}
    ...
    Then 
    \begin{equation*}
        \pi_0\pnbr(\underline{k}) = \begin{cases}
            0 & \mathrm{char}\, k \neq 2 \\
            \ZZ & \mathrm{char}\, k = 2 \,.
        \end{cases}
    \end{equation*}
