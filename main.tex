...
\end{definition}
\begin{remark}
    Suppose $ (X,\lambda, Y, p) $ is an object of $ \mathrm{qSch}^{C_2} $ and $ U \to Y $ is a quasi-compact open subscheme. 
...
\begin{notation}\label{notation:scheme_involution_basechange}
    ...
    \Lucy{Is $ \pi^*U $ an object of $ \mathrm{qSch}^{C_2} $ over $ (X, \sigma, Y, \pi) $? If not, does that matter? If $ j $ is flat, $ X \times_Y U $ agrees with derived fiber product and $ U $ inherits $ C_2 $-$ \EE_\infty $ ``structure sheaf'' so can still define Poincaré module category.} 
