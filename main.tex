...
\end{lemma}
\begin{proof}
    Recall \cite[Theorem 5.8]{MR3190610} that the sheaf $ \mathbf{M}_{A^e} $ is locally geometric, i.e. there exists a filtered system $ \mathbf{M}_{A^e} = \colim_i \mathbf{M}_{A^e}^i $ where each $ \mathbf{M}_{A^e}^i \to \mathbf{M}_{A^e} $ is a monomorphism and each $ \mathbf{M}_{A^e}^i $ is $ n_i $-geometric for some $ n_i $. 
    \Lucyil{Does pushforward preserve filtered colimits? \cite[IV.5]{SGA4} shows that $ R^q f_* $ where $ f $ is a coherent morphism of coherent topoi preserves filtered colimits...of abelian sheaves! Appendix A \href{https://arxiv.org/abs/2407.19920}{here} seems to suggest that it suffices to assume that $ X $, $ Y $ are qcqs and $ f $ is quasi-compact; \cite[Corollary 7.3.5.4]{HTT}. Return to this later; maybe ask Peter Haine. Don't forget to modify assumptions as necessary!} 
