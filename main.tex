...
\begin{lemma}\label{lemma:hermitian_moduli_to_ordinary_moduli_is_geometric}
    ...
    Then the morphism $ \mathbf{M}^h_A \to \pi_* \mathbf{M}_{A^e} $ of Observation \ref{obs:from_moduli_of_hermitian_objects_to_moduli_of_objects} is locally of finite presentation in the sense of \cite[\S4.3]{MR3190610}. 
\end{lemma}
\begin{proof}
    ...
    \Lucyil{WORK IN PROGRESS: map is locally of finite presentation. REMAINING: last two bullet points} 
    We now turn to the proof of the latter half of the statement. 
    By the proof of the first half and \cite[Theorem 5.2]{MR3190610}, it suffices to show that $ \hom_{N_R^{C_2}(\widetilde{p}^*A)}\left(\widetilde{p}^*A, N^{C_2}_R(M^\vee)\right) $ is a perfect $ S $-module. 
