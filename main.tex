...
\Lucy{explain why using good quotients/choice of quotient is useful.}
...
\end{corollary}
\begin{proof}
Since $\theta:\mathrm{Mod}_{(\Mod_A^\omega, \Qoppa_A)}(\Catpidem)\to \mathrm{Mod}_{\mathrm{Mod}_{A^e}}(\Catex)$ is symmetric monoidal and conservative, it induces a map $ \theta^\simeq \colon \pnbr(A) \to \mathrm{br}(A^e) $ on the groupoid core of invertible objects. 
...
\begin{definition}\label{defn:Azumaya_genuine_anti-inv}
    ...
    \begin{enumerate}[label=(\alph*)]
        ...
        \item \label{defn_item:Azumaya_gen_inv_conn_map} An $ A $-linear map $ P \to A^{tC_2} $ and an $ A^\op $-linear map $ \overline{P} \to A^{tC_2} $. 
        Here we regard $ A^{tC_2} $, which is canonically a $ (A \otimes A)^{tC_2} $-module, as an $ A $-module (resp. $ A^\op $-module) via the twisted Tate-valued diagonal $ A \to (A \otimes A^\op)^{tC_2} $ (resp. $ A^\op \to (A \otimes A^\op)^{tC_2} $). 
        \item \label{defn_item:Azumaya_gen_inv_geom_fixpt} An equivalence of $ A \otimes_R A^\op $-modules 
        \begin{equation*}
...
\begin{proof}[Proof of Proposition \ref{prop:mod_over_azumaya_geninv_is_invertible}]
    The first statement follows from unravelling definitions; we prove the second statement. 
